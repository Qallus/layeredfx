...
export async function GET(request:Request){try{await access(request);const state=await readBlog(),type=new URL(request.url).searchParams.get('type')==='email_template'?'email_template':'blog_post';return Response.json({...state,items:state.items.filter(item=>item.content_type===type)},{headers:{'Cache-Control':'private, no-store'}});}catch(e){return errorResponse(e);}}
