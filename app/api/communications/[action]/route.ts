...
import {emailConfigured,sendStaffEmail,staffEmailInput} from '@/lib/communications/email';
...
export async function GET(request:Request,context:Context){try{
 ...
 if(action==='capabilities')return Response.json({...capabilities(),email:emailConfigured(),canSend:actor.role!=='viewer'},{headers});
...
export async function POST(request:Request,context:Context){try{
 ...
 if(action==='email'){const result=await sendStaffEmail(staffEmailInput(body),actor);return Response.json({id:result.id,message:'Accepted by the email provider. Delivery is not yet confirmed.'},{status:201,headers});}
 if(action==='sms'){
