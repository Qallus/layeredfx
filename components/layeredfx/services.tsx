...
export function Services() {
  ...
  return <section id="services" className="lfx-section lfx-container">
    ...
    <div className="lfx-filter-row" role="group" aria-label="Filter services">{filters.map(item => <button key={item.id} aria-pressed={filter === item.id} onClick={event => { setFilter(item.id); event.currentTarget.scrollIntoView({ block: "nearest", inline: "nearest" }); event.currentTarget.closest("section")?.querySelector(".lfx-service-grid")?.scrollTo({ left: 0 }); }}>{item.label}</button>)}</div>
