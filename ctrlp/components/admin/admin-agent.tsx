// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-agent.tsx
"use client";
...
import { Bell,Bot,ChevronDown,Clock,Copy,FileText,Loader2,Mail,MessageSquare,Plus,Send,Settings2,Sparkles,Wrench,X,Zap } from "lucide-react";
...
function ToolCallCard({ call }: {
    ...
    return (<div className={cn("my-1 rounded-lg border text-[12px]", isError ? "border-red-500/30 bg-red-500/5" : "border-primary/20 bg-primary/5")}>
      ...
      {open && (<div className="border-t border-primary/10 px-3 pb-2 pt-2 space-y-1.5">
          <div>
...
function DraftBlock({ content }: {
    ...
    return (<div className="my-2 rounded-lg border border-primary/30 bg-primary/5">
...
function renderContent(content: string) {
    ...
    return parts.map((part, i) => {
        ...
        if (match)
            return <DraftBlock key={i} content={match[1].trim()}/>;
        return (<span key={i}>
...
export function AdminAgent() {
    ...
    async function sendMessage(text?: string) {
        ...
        try {
            ...
            const res = await sourceFetch("/api/ctrlp/admin/agent", {
                method: "POST",
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="flex h-[calc(100vh-48px)] ">
          {authState === "checking" && (<div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
              <Loader2 className="mr-2 h-4 w-4 animate-spin"/> Checking access...
            </div>)}
          {authState === "denied" && (<div className="flex flex-1 items-center justify-center p-6">
          ...
          {authState === "allowed" && (<div className="flex flex-1 overflow-hidden">
              {/* Skills panel */}
                  ...
                  <div className="space-y-4">
                    {SKILLS.map((group) => (<div key={group.category}>
                        ...
                        <div className="space-y-1">
                          {group.items.map((skill) => (<button key={skill.label} disabled={thinking} onClick={() => sendMessage(skill.prompt)} className="flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-[12px] text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground disabled:opacity-40">
              ...
              <div className="flex flex-1 flex-col overflow-hidden">
                ...
                <div className="flex items-center gap-2 border-b bg-background/50 px-4 py-2">
                  {!skillsOpen && (<Button variant="ghost" size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setSkillsOpen(true)}>
                  ...
                  <Select value={model} onValueChange={setModel}>
                    ...
                    <SelectContent>
                      {AGENT_MODELS.map((m) => (<SelectItem key={m.id} value={m.id} disabled={m.disabled}>
                          <span className="flex items-center gap-2">
                            {m.label}
                            <Badge className={cn("h-4 px-1 text-[9px]", m.badge === "Soon" ? "bg-muted text-muted-foreground" :
                    m.badge === "Fast" ? "bg-green-500/20 text-green-700 dark:text-green-400" :
                        "bg-primary/20 text-foreground")}>{m.badge}</Badge>
                            {m.tools && <Badge className="h-4 bg-blue-500/20 px-1 text-[9px] text-blue-700 dark:text-blue-400">Tools</Badge>}
                          </span>
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                  {!selectedModelConfig?.tools && (<span className="text-[11px] text-amber-600 dark:text-amber-400">⚠ This model doesn&apos;t support tools</span>)}
                  {messages.length > 0 && (<Button variant="ghost" size="sm" className="ml-auto h-7 text-xs text-muted-foreground" onClick={() => setMessages([])}>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  {messages.length === 0 && !thinking && (<div className="flex h-full flex-col items-center justify-center gap-4 text-center">
                ...
                ].map((s) => (<button key={s.label} onClick={() => sendMessage(s.prompt)} className="rounded-lg border bg-card/50 px-3 py-2 text-left text-sm transition-colors hover:border-primary/40 hover:bg-primary/5">
                            {s.label}
                          </button>))}
                      </div>
                    </div>)}
                  ...
                  <div className="space-y-4">
                    {messages.map((msg, i) => (<div key={i} className={cn("flex", msg.role === "user" ? "justify-end" : "justify-start")}>
                        {msg.role === "assistant" && (<div className="mr-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border bg-primary/10">
                            <Bot className="h-3.5 w-3.5 text-primary"/>
                          </div>)}
                        <div className="max-w-[80%]">
                          {msg.role === "assistant" && msg.toolCalls?.length ? (<div className="mb-2 space-y-1">
                              {msg.toolCalls.map((tc) => (<ToolCallCard key={tc.id} call={tc}/>))}
                            </div>) : null}
                          <div className={cn("rounded-xl px-3.5 py-2.5 text-sm leading-relaxed", msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-muted text-foreground")}>
                ...
                <div className="border-t bg-background/80 p-3 backdrop-blur">
                  <div className="flex gap-2">
                    <Textarea ref={textareaRef} className="min-h-[52px] max-h-[140px] resize-none text-sm" placeholder="Ask about orders, draft an SMS, write a blog post, analyze revenue..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === "Enter" && !e.shiftKey) {
                    e.preventDefault();
                    sendMessage();
                }
            }} disabled={thinking}/>
                    <Button className="h-auto self-stretch px-3" onClick={() => sendMessage()} disabled={thinking || !input.trim()} aria-label="Send">
                      {thinking ? <Loader2 className="h-4 w-4 animate-spin"/> : <Send className="h-4 w-4"/>}
                    </Button>
                  </div>
                  <div className="mt-1.5 flex items-center gap-3">
                    <span className="text-[11px] text-muted-foreground">Enter to send · Shift+Enter for new line</span>
                    {selectedModelConfig?.tools && (<span className="text-[11px] text-primary/70">⚡ Tool calling enabled — agent can read data and send SMS/email</span>)}
                  </div>
                </div>
              </div>
            </div>)}
