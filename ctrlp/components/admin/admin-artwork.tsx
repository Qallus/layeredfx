// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-artwork.tsx
"use client";
...
import { getCurrentAdminProfile,loadAdminDashboardData,updateAdminArtworkReview,uploadAdminArtwork } from "@/ctrlp/lib/admin/admin-api";
import type { AdminDashboardData,ArtworkFile,Proof } from "@/ctrlp/lib/admin/types";
...
function statusTone(status: string) {
    if (["approved", "in_production"].includes(status))
        return "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300";
    if (["proof_sent", "waiting_for_file_review"].includes(status))
        return "border-primary/20 bg-primary/15 text-lime-800 dark:text-lime-200";
    if (["needs_changes", "rejected"].includes(status))
...
export function AdminArtwork() {
    ...
    async function refresh(openRecord?: {
        kind: "artwork" | "proof";
        id: string;
    }) {
        ...
        <main className="px-4 py-5  ">
          ...
          {authState === "allowed" && (<>
              ...
              <div className="mb-4 flex gap-1 rounded-lg border bg-muted/40 p-1 w-fit">
                <button onClick={() => setActiveTab("artwork")} className={cn("rounded-md px-4 py-1.5 text-xs font-medium transition-colors", activeTab === "artwork" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
                  Artwork & proofs
                </button>
                <button onClick={() => setActiveTab("designs")} className={cn("flex items-center gap-1.5 rounded-md px-4 py-1.5 text-xs font-medium transition-colors", activeTab === "designs" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
                  Designer drafts
                  {drafts.filter((d) => d.status === "ready_for_review").length > 0 && (<span className="rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400">
                      {drafts.filter((d) => d.status === "ready_for_review").length} to review
                    </span>)}
                </button>
              </div>

              {activeTab === "artwork" && (<section className="mb-4 grid gap-4 xl:grid-cols-[1fr_360px]">
                  <Card>
                    ...
                    return (<TableRow key={`${record.kind}-${record.id}`} className="cursor-pointer hover:bg-accent/45" onClick={() => setSelectedRecord(record)}>
              ...
              {activeTab === "designs" && (<section className="mb-4">
                  <Card>
                    ...
                    return (<TableRow key={draft.id} className="hover:bg-accent/45">
                                ...
                                <TableCell>
                                  {linkedOrder ? (<span className="font-mono text-xs">#{linkedOrder.order_number}</span>) : (<span className="text-xs text-muted-foreground">No order</span>)}
                                </TableCell>
                                <TableCell className="pr-4">{formatDate(draft.last_saved_at)}</TableCell>
                              </TableRow>);
                })}
                          {!drafts.length && (<TableRow><TableCell colSpan={6} className="p-6 text-center text-muted-foreground">No designer drafts yet.</TableCell></TableRow>)}
                        </TableBody>
                      </Table>
                    </CardContent>
                  </Card>
...
function getRecordLinks(record: ArtworkRecord, data: {
    orders: AdminDashboardData["orders"];
    orderItems: AdminDashboardData["orderItems"];
...
function ArtworkStat({ label, value, hint }: {
    label: string;
    value: string;
    hint: string;
}) {
...
function MiniRow({ title, detail, value }: {
    title: string;
    detail: string;
    value: string;
}) {
...
function UploadArtworkSheet({ open, onOpenChange, data, onUploaded }: {
    open: boolean;
...
function ArtworkReviewSheet({ record, data, open, onOpenChange, onSaved }: {
