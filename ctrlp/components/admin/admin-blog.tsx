// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-blog.tsx
"use client";
...
function BlogBlockPreview({ block }: {
    block: BlogBlock;
}) {
    const { type, props } = block;
...
function BlogBlockSettings({ block, onChange }: {
    block: BlogBlock;
    onChange: (props: Partial<BlogBlockProps>) => void;
}) {
    const p = block.props;
    ...
    const alignButtons = (val: string | undefined) => (<div className="flex gap-1">
      {(["left", "center", "right"] as const).map((a) => (<Button key={a} size="icon" variant="outline" className={cn("h-7 w-7", val === a && "border-primary bg-primary/10")} onClick={() => onChange({ align: a })}>
          {a === "left" ? <AlignLeft className="h-3.5 w-3.5"/> : a === "center" ? <AlignCenter className="h-3.5 w-3.5"/> : <AlignRight className="h-3.5 w-3.5"/>}
        </Button>))}
    </div>);
    const colorPicker = (label: string, key: keyof BlogBlockProps, fallback: string) => (<div>{lbl(label)}<div className="flex gap-1"><input type="color" value={String(p[key] ?? fallback)} onChange={(e) => onChange({ [key]: e.target.value } as Partial<BlogBlockProps>)} className="h-7 w-8 cursor-pointer rounded border p-0.5"/><Input className={ic} value={String(p[key] ?? "")} onChange={(e) => onChange({ [key]: e.target.value } as Partial<BlogBlockProps>)}/></div></div>);
    // Section spacing — shown at bottom of every block
    const sectionControls = (<div className="mt-3 space-y-2 border-t pt-3">
...
function CalendarView({ posts, month, onMonth }: {
    posts: ContentItem[];
    month: Date;
    onMonth: (d: Date) => void;
}) {
            ...
            return (<div key={ci} className={cn("min-h-[80px] bg-card p-1.5", !day && "bg-muted/30")}>
              {day && (<>
                  ...
                  <div className="mt-1 space-y-0.5">
                    {dayPosts.slice(0, 2).map((p) => (<div key={p.id} className={cn("truncate rounded px-1 py-0.5 text-[10px] font-medium", STATUS_STYLE[p.status])}>{p.title}</div>))}
...
export function AdminBlog() {
    ...
    function buildPublishedAt() {
        ...
        if (form.publish_ampm === "PM" && h !== 12)
            h += 12;
        if (form.publish_ampm === "AM" && h === 12)
            h = 0;
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="">
          ...
          {/* ══ LIST PAGE ══ */}
          {authState === "allowed" && page === "list" && (<div className="p-5 space-y-5">
            ...
            ].map(({ label, value, sub, icon: Icon, s }) => (<div key={label} onClick={() => setStatusFilter(s)} className="cursor-pointer rounded-xl border bg-card p-4 hover:border-primary/40 transition-colors">
                    <div className="flex items-start justify-between">
                      <div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex flex-wrap gap-1">
                  {(["all", "draft", "scheduled", "published", "archived"] as const).map((s) => (<button key={s} onClick={() => setStatusFilter(s)} className={cn("rounded-full border px-3 py-1 text-[12px] font-medium capitalize transition-all", statusFilter === s ? "border-primary/60 bg-primary/10 text-foreground" : "border-transparent bg-muted text-muted-foreground hover:border-border")}>
              ...
              {/* ── Cards ── */}
              {view === "cards" && (<div className="grid gap-5 sm:grid-cols-2">
                  {visiblePosts.map((post) => (<div key={post.id} className="group overflow-hidden rounded-xl border bg-card transition-shadow hover:shadow-md">
                      ...
                      <div className="p-4">
                        ...
                        {(post.categories?.length ?? 0) > 0 && (<div className="mb-3 flex flex-wrap gap-1">
                  ...
                  {!loading && visiblePosts.length === 0 && (<div className="col-span-full py-20 text-center">
              ...
              {view === "list" && (<div className="divide-y rounded-xl border bg-card">
                  {visiblePosts.map((post) => (<div key={post.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30">
              ...
              {view === "table" && (<div className="overflow-x-auto rounded-xl border bg-card">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {visiblePosts.map((post) => (<tr key={post.id} className="hover:bg-muted/20">
          ...
          {/* ══ EDITOR PAGE ══ */}
          {authState === "allowed" && page === "editor" && (<div className="p-5 space-y-5">
              <div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl border bg-card p-4">
                  ...
                  {gallery.map((g, gi) => (<div key={gi} className="mb-1 flex items-center gap-1">
              ...
              {showAdvancedUrl && (<div className="rounded-xl border bg-card p-4 space-y-3">
                  <div>
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mb-3">
                  {(["simple", "html", "wysiwyg"] as EditorTab[]).map((t) => (<button key={t} onClick={() => {
                    if (t === "html" && editorTab === "simple") {
                        const mw = canvasWidth === "full" ? undefined : canvasWidth === "wide" ? "1200px" : canvasWidth === "article" ? "880px" : "640px";
                        setHtmlContent(blogBlocksToHtml(blogBlocks, mw));
                    }
                    if (t === "wysiwyg" && wysiwygRef.current) {
                        const mw = canvasWidth === "full" ? undefined : canvasWidth === "wide" ? "1200px" : canvasWidth === "article" ? "880px" : "640px";
                        wysiwygRef.current.innerHTML = editorTab === "simple" ? blogBlocksToHtml(blogBlocks, mw) : htmlContent;
                    }
                    setEditorTab(t);
                }} className={cn("rounded-md px-4 py-1.5 text-sm font-medium transition-colors", editorTab === t ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground")}>
                      {t === "simple" ? "Block Builder" : t === "html" ? "Advanced HTML" : "WYSIWYG"}
                    </button>))}
                </div>

                {/* Block Builder (Simple) */}
                {editorTab === "simple" && (<div className="overflow-hidden rounded-xl border">
                ...
                ] as const).map(({ id, label }) => (<button key={id} onClick={() => setCanvasWidth(id)} className={cn("rounded px-2 py-0.5 text-[11px] font-medium transition-colors", canvasWidth === id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground")}>{label}</button>))}
                      </div>
                    ...
                    <div className="flex h-[680px]">
                      ...
                      <div className="w-44 shrink-0 overflow-y-auto border-r bg-muted/20 p-2">
                        ...
                        <div className="space-y-1">
                          {BLOG_BLOCK_DEFS.map(({ type, label, icon: Icon, defaults }) => (<button key={type} onClick={() => addBlogBlock(type, defaults)} className="flex w-full items-center gap-2 rounded-lg border border-transparent bg-card px-2 py-2 text-[12px] font-medium transition-colors hover:border-primary/30 hover:bg-accent">
                      ...
                      <div className="flex-1 overflow-y-auto bg-white dark:bg-background">
                        <div className={cn("min-h-full p-6", canvasWidth !== "full" && "flex justify-center")}>
                          <div className={cn("w-full", canvasWidth === "wide" && "max-w-[1200px]", canvasWidth === "article" && "max-w-[880px]", canvasWidth === "narrow" && "max-w-[640px]")}>
                            {blogBlocks.length === 0 ? (<div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-3 text-muted-foreground">
                                ...
                                {blogBlocks.map((block, idx) => (<div key={block.id} draggable onDragStart={() => setBlogDragIdx(idx)} onDragOver={(e) => { e.preventDefault(); setBlogDropIdx(idx); }} onDrop={() => {
                            if (blogDragIdx !== null && blogDragIdx !== idx)
                                moveBlogBlock(blogDragIdx, idx);
                            setBlogDragIdx(null);
                            setBlogDropIdx(null);
                        }} onDragEnd={() => { setBlogDragIdx(null); setBlogDropIdx(null); }} onClick={() => setSelectedBlogBlockId(block.id)} className={cn("group relative cursor-pointer rounded-lg border-2 p-3 transition-colors", selectedBlogBlockId === block.id ? "border-primary bg-primary/5" : "border-transparent hover:border-primary/30", blogDropIdx === idx && blogDragIdx !== null && "border-primary/60 bg-primary/10")}>
                                    <div className="absolute -left-1 top-1/2 -translate-y-1/2 cursor-grab opacity-0 group-hover:opacity-100">
                                      <GripVertical className="h-4 w-4 text-muted-foreground"/>
                                    </div>
                                    <button onClick={(e) => { e.stopPropagation(); removeBlogBlock(block.id); }} className="absolute right-1 top-1 rounded p-0.5 opacity-0 hover:bg-red-500/10 hover:text-red-500 group-hover:opacity-100">
                                      <X className="h-3 w-3"/>
                                    </button>
                ...
                {/* Advanced HTML */}
                {editorTab === "html" && (<div className="overflow-hidden rounded-xl border">
                    <Textarea value={htmlContent} onChange={(e) => setHtmlContent(e.target.value)} className="min-h-[400px] resize-none rounded-xl border-0 font-mono text-[13px] leading-relaxed focus-visible:ring-0" placeholder="<p>Write raw HTML here…</p>"/>
                  </div>)}

                {/* WYSIWYG */}
                {editorTab === "wysiwyg" && (<div className="overflow-hidden rounded-xl border">
                    <div className="flex flex-wrap items-center gap-1 border-b px-3 py-2">
                      {[["Bold", "bold"], ["Italic", "italic"], ["Underline", "underline"]].map(([l, c]) => (<button key={c} onMouseDown={(e) => { e.preventDefault(); doWysiwyg(c); }} className="rounded border bg-muted px-2 py-1 text-[12px] font-medium hover:bg-accent">{l}</button>))}
                      {[["Left", "justifyLeft"], ["Center", "justifyCenter"], ["Right", "justifyRight"]].map(([l, c]) => (<button key={c} onMouseDown={(e) => { e.preventDefault(); doWysiwyg(c); }} className="rounded border bg-muted px-2 py-1 text-[12px] hover:bg-accent">{l}</button>))}
                      {[["H2", "<h2>"], ["H3", "<h3>"], ["P", "<p>"]].map(([l, v]) => (<button key={l} onMouseDown={(e) => { e.preventDefault(); doWysiwyg("formatBlock", v); }} className="rounded border bg-muted px-2 py-1 text-[12px] hover:bg-accent">{l}</button>))}
                      ...
                      <button onMouseDown={(e) => { e.preventDefault(); const u = window.prompt("Link URL:"); if (u)
                doWysiwyg("createLink", u); }} className="rounded border bg-muted px-2 py-1 text-[12px] hover:bg-accent">Link</button>
                    </div>
