// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-content.tsx
"use client";
...
import type { AdminUser,ContentItem,ContentStatus,ContentType,GalleryItem } from "@/ctrlp/lib/admin/types";
...
import { AlignLeft,Archive,ArrowRight,BookOpen,Bot,Calendar,ChevronLeft,ChevronRight,Edit2,Facebook,GripVertical,Image,Instagram,Layers,LayoutGrid,LayoutList,Linkedin,List,Mail,Minus,Pin,Plus,Send,Trash2,Type,Video,X,Zap } from "lucide-react";
...
function htmlToBlocks(html: string): ContentBlock[] {
    ...
    if (typeof window === "undefined")
        return [{ id: uid(), type: "p", content: html }];
    ...
    doc.body.childNodes.forEach((node) => {
        ...
        if (tag === "h1")
            ...
            blocks.push({ id, type: "h3", content: el.textContent || "" });
            ...
            if (img)
                blocks.push({ id, type: "img", content: img.getAttribute("src") || "", alt: img.getAttribute("alt") || "", caption: cap?.textContent || "" });
        }
...
async function fetchItems(type = "all", status = "all"): Promise<ContentItem[]> {
    ...
    const res = await sourceFetch(`/api/admin/content?${params}`, {
...
function BlockEditor({ blocks, onChange }: {
    ...
    return (<div className="space-y-2">
      {blocks.map((block, i) => (<div key={block.id} draggable onDragStart={() => setDragIdx(i)} onDragOver={(e) => { e.preventDefault(); setDropIdx(i); }} onDragEnd={() => { setDragIdx(null); setDropIdx(null); }} onDrop={() => drop(i)} className={cn("group flex gap-2 rounded-lg border bg-card/50 p-2 transition-all", dropIdx === i && dragIdx !== i && "border-primary/60 bg-primary/5")}>
          ...
          <div className="flex-1 space-y-1.5">
            {block.type === "hr" ? (<div className="my-2 border-t border-border"/>) : block.type === "img" ? (<div className="space-y-1.5">
              ...
              </div>) : block.type === "video" ? (<div className="space-y-1.5">
                ...
                <Input placeholder="Caption (optional)" value={block.caption || ""} onChange={(e) => update(block.id, { caption: e.target.value })} className="text-xs"/>
              </div>) : block.type === "list" ? (<Textarea placeholder="One item per line" value={block.content} onChange={(e) => update(block.id, { content: e.target.value })} className={cn("min-h-[80px] resize-none text-sm")}/>) : (<Textarea placeholder={block.type === "quote" ? "Quote text…" : block.type === "h1" ? "Heading 1…" : block.type === "h2" ? "Heading 2…" : block.type === "h3" ? "Heading 3…" : "Paragraph text…"} value={block.content} onChange={(e) => update(block.id, { content: e.target.value })} className={cn("min-h-[52px] resize-none border-0 bg-transparent p-0 shadow-none focus-visible:ring-0", block.type === "h1" && "text-2xl font-bold", block.type === "h2" && "text-xl font-semibold", block.type === "h3" && "text-lg font-medium", block.type === "quote" && "border-l-2 border-primary/40 pl-3 italic")}/>)}
          </div>
          <div className="flex flex-col gap-1 pt-1">
      ...
      <div className="relative">
        ...
        {addOpen && (<div className="absolute left-0 top-8 z-10 w-52 rounded-lg border bg-card shadow-lg">
            {BLOCK_TYPES.map(({ type, label, icon: Icon }) => (<button key={type} type="button" onClick={() => addBlock(type)} className="flex w-full items-center gap-2.5 px-3 py-2 text-left text-[13px] hover:bg-accent">
...
// ─── Calendar View ─────────────────────────────────────────────────────────────
function CalendarView({ items, month, onMonth }: {
    items: ContentItem[];
    month: Date;
    onMonth: (d: Date) => void;
}) {
    ...
    return (<div>
      ...
      <div className="grid grid-cols-7 gap-px overflow-hidden rounded-lg border bg-border">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (<div key={d} className="bg-card px-2 py-1.5 text-center text-[11px] font-semibold text-muted-foreground">{d}</div>))}
        {cells.map((day, ci) => {
            const isToday = day !== null && today.getDate() === day && today.getMonth() === mon && today.getFullYear() === year;
            const dayItems = day ? (byDay[day] ?? []) : [];
            return (<div key={ci} className={cn("min-h-[80px] bg-card p-1.5", !day && "bg-muted/30")}>
              {day && (<>
                  ...
                  <div className="mt-1 space-y-0.5">
                    {dayItems.slice(0, 3).map((item) => {
                        ...
                        return (<div key={item.id} className={cn("truncate rounded px-1 py-0.5 text-[10px] font-medium", meta.color)}>{item.title}</div>);
                    })}
...
export function AdminContent() {
    ...
    function switchEditorTab(tab: "visual" | "html") {
        ...
        if (tab === "visual")
            setBlocks(htmlToBlocks(htmlContent));
        setEditorTab(tab);
    }
    async function save(statusOverride?: ContentStatus) {
        ...
        try {
            const result = await apiCall(editItem ? "PATCH" : "POST", payload) as {
                item: ContentItem;
            };
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="">
          {authState === "checking" && (<div className="flex h-[calc(100vh-48px)] items-center justify-center text-sm text-muted-foreground">
              <Zap className="mr-2 h-4 w-4 animate-pulse text-primary"/> Loading…
            </div>)}
          {authState === "denied" && (<div className="flex h-[calc(100vh-48px)] items-center justify-center p-6">
          ...
          {authState === "allowed" && (<div className="p-5">
              {/* Page header */}
              ...
              <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
                ...
                return (<button key={t} onClick={() => setActiveType(t)} className={cn("rounded-lg border p-3 text-left transition-colors hover:border-primary/40", activeType === t && "border-primary/60 bg-primary/5")}>
              ...
              <div className="mb-4 flex flex-wrap items-center gap-2">
                ...
                const Icon = icons[v];
                return (<Button key={v} variant="outline" size="icon" className={cn("h-8 w-8", view === v && "border-primary/60 bg-primary/10")} onClick={() => setView(v)}>
              ...
              {/* ── Cards view ── */}
              {view === "cards" && (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {visibleItems.map((item) => (<div key={item.id} className="group overflow-hidden rounded-xl border bg-card transition-shadow hover:shadow-md">
                      ...
                      <div className="p-3">
                        ...
                        <div className="mt-2 flex items-center gap-1.5">
                          ...
                          <div className="relative">
                            ...
                            {convertOpen === item.id && (<div className="absolute left-0 top-7 z-10 w-44 rounded-lg border bg-card shadow-lg">
                                {CONVERT_OPTIONS.map((ct) => {
                            const m = TYPE_META[ct];
                            const Icon = m.icon;
                            return (<button key={ct} type="button" onClick={() => { openNew(ct, item); setConvertOpen(null); }} className="flex w-full items-center gap-2 px-3 py-1.5 text-[12px] hover:bg-accent">
                  ...
                  {!loading && visibleItems.length === 0 && (<div className="col-span-full py-16 text-center text-sm text-muted-foreground">No content yet. <button className="text-primary underline" onClick={() => openNew()}>Create your first post.</button></div>)}
                </div>)}

              {/* ── List view ── */}
              {view === "list" && (<div className="divide-y rounded-xl border bg-card">
                  {visibleItems.map((item) => {
                    const meta = TYPE_META[item.content_type];
                    const Icon = meta.icon;
                    return (<div key={item.id} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  {!loading && visibleItems.length === 0 && (<div className="py-10 text-center text-sm text-muted-foreground">No content found.</div>)}
                </div>)}

              {/* ── Table view ── */}
              {view === "table" && (<div className="overflow-x-auto rounded-xl border bg-card">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {visibleItems.map((item) => (<tr key={item.id} className="hover:bg-muted/30">
              ...
              {/* ── Calendar view ── */}
              {view === "calendar" && (<CalendarView items={visibleItems} month={calMonth} onMonth={setCalMonth}/>)}
            </div>)}
        </main>
          ...
          <SheetContent className="flex w-full max-w-3xl flex-col gap-0 overflow-hidden p-0 sm:max-w-3xl">
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="flex flex-1 flex-col overflow-hidden">
                ...
                {!editItem && (<div className="shrink-0 border-b bg-muted/30 px-5 py-3">
                    <div className="flex flex-wrap gap-2">
                      {(Object.keys(TYPE_META) as ContentType[]).map((ct) => {
                const { label, color, icon: Icon } = TYPE_META[ct];
                return (<button key={ct} type="button" onClick={() => setForm((f) => ({ ...f, content_type: ct }))} className={cn("flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12px] font-medium border transition-all", form.content_type === ct ? "border-primary/60 " + color : "border-transparent bg-muted text-muted-foreground hover:border-border")}>
                ...
                <div className="shrink-0 border-b bg-muted/20 px-5">
                  <div className="flex gap-1 py-2">
                    {([["visual", "Visual"], ["html", "HTML"]] as const).map(([tab, label]) => (<button key={tab} type="button" onClick={() => switchEditorTab(tab)} className={cn("rounded px-3 py-1 text-[12px] font-medium transition-colors", editorTab === tab ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                        {label}
                      </button>))}
                    {charLimit && (<span className={cn("ml-auto flex items-center text-[11px]", charCount > charLimit ? "text-red-500" : "text-muted-foreground")}>
                ...
                <div className="flex-1 overflow-y-auto p-5">
                  {/* Title */}
                  <div className="mb-4">
                    <Input placeholder={form.content_type === "email_template" ? "Template name…" : "Title…"} value={form.title} onChange={(e) => setForm((f) => ({
            ...
            slug: f.slug || slugify(e.target.value),
                  ...
                  {form.content_type === "email_template" && (<div className="mb-4 space-y-2 rounded-lg border bg-card/50 p-3">
                  ...
                  {/* HTML editor */}
                  {editorTab === "html" && (<Textarea value={htmlContent} onChange={(e) => setHtmlContent(e.target.value)} className="min-h-[400px] font-mono text-[12px]" placeholder="<p>Enter HTML content…</p>"/>)}
                </div>
              </div>
              ...
              <div className="w-[260px] shrink-0 overflow-y-auto border-l bg-card/40 px-4 py-4 space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  {(form.status === "scheduled" || form.status === "published") && (<Input type="datetime-local" value={form.published_at} onChange={(e) => setForm((f) => ({ ...f, published_at: e.target.value }))} className="h-8 text-xs"/>)}
                  <div className="flex gap-1.5 pt-1">
                    <Button size="sm" className="flex-1 h-7 text-xs" disabled={saving} onClick={() => save()}>
                      {saving ? "Saving…" : "Save"}
                    </Button>
                    {form.status !== "published" && (<Button size="sm" variant="outline" className="flex-1 h-7 text-xs gap-1" disabled={saving} onClick={() => save("published")}>
                        <Send className="h-3 w-3"/> Publish
                      </Button>)}
                  </div>
                  {editItem && (<Button size="sm" variant="ghost" className="w-full h-7 text-xs text-red-500 hover:text-red-600" onClick={() => { deleteItem(editItem.id); setEditorOpen(false); }}>
                      <Trash2 className="mr-1 h-3 w-3"/> Delete
                    </Button>)}
                </div>

                {/* Blog post fields */}
                {form.content_type === "blog_post" && (<div className="space-y-2">
                ...
                {/* Social card fields */}
                {form.content_type.startsWith("social_") && (<div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  {gallery.map((g, gi) => (<div key={gi} className="rounded border bg-background/50 p-2 space-y-1">
                      <div className="flex items-center gap-1">
                ...
                {(editItem?.content_type === "blog_post" || form.content_type === "blog_post") && editItem && (<div className="space-y-2">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Convert to</p>
                    <div className="space-y-1">
                      {CONVERT_OPTIONS.map((ct) => {
                const m = TYPE_META[ct];
                const Icon = m.icon;
                return (<Button key={ct} type="button" variant="outline" size="sm" className="w-full justify-start h-7 gap-1.5 text-xs" onClick={() => { openNew(ct, editItem); }}>
                            <Icon className="h-3.5 w-3.5"/>{m.label}
                          </Button>);
            })}
                    </div>
