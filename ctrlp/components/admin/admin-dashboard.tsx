// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-dashboard.tsx
"use client";
...
import { Separator } from "@/ctrlp/components/ui/separator";
...
import { getCurrentAdminProfile,loadAdminDashboardData,markMessageRead,updateProductionJobStatus,} from "@/ctrlp/lib/admin/admin-api";
import type { AdminDashboardData,Order } from "@/ctrlp/lib/admin/types";
import { getSupabaseBrowserClient } from "@/ctrlp/lib/supabase/browser";
import { cn } from "@/ctrlp/lib/utils";
import { AlertTriangle,CircleDollarSign,Trophy,Zap } from "lucide-react";
...
export function AdminDashboard() {
    ...
    const revenue = useMemo(() => payments.filter((payment) => payment.status === "paid").reduce((sum, payment) => sum + numberValue(payment.amount), 0), [payments]);
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="px-4 py-5  ">
          {authState === "checking" && (<Card className="mb-5">
              <CardContent className="p-5 text-sm text-muted-foreground">Checking admin access...</CardContent>
            </Card>)}

          {authState === "denied" && (<Card className="mb-5 border-red-500/30">
          ...
          {authState === "allowed" && (<>
              ...
              <section className="mb-4 grid gap-4 xl:grid-cols-[1fr_1fr]">
                <Card>
                  ...
                  <CardContent>
                    <div className="grid gap-2 sm:grid-cols-4">
                      {["file_check", "proof_pending", "printing", "on_hold"].map((status) => (<div key={status} className={cn("rounded-lg border p-4 text-center", statusTone(status))}>
                ...
                return (<div key={status} className="rounded-lg border bg-background/35 p-2.5">
              ...
              <Card className="mb-4">
                ...
                <CardContent className="space-y-2">
                  {alerts.length ? alerts.map(([level, text]) => (<div key={text} className="flex items-center gap-3 rounded-lg border bg-background/40 px-3 py-2 text-sm">
                      ...
                      <span>{text}</span>
                    </div>)) : (<div className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">No live operational alerts.</div>)}
              ...
              <section className="mb-4 grid gap-4 xl:grid-cols-[1fr_360px]">
                <Card>
                  ...
                  <CardContent className="p-0">
                    <Table>
                      ...
                      <TableBody>
                        {orders.map((order) => (<TableRow key={order.id} className="cursor-pointer" onClick={() => setSelectedOrder(order)}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    {unreadMessages.slice(0, 5).map((message) => (<div key={message.id} className="rounded-lg border bg-background/35 p-3">
              ...
              <section className="grid gap-4 xl:grid-cols-2">
                <LeaderCard icon={<Trophy className="h-5 w-5 text-primary"/>} title="Top products" subtitle="Revenue-weighted live order mix">
                  {productLeaders.map((product, index) => (<LeaderRow key={product.name} index={index + 1} name={product.name} detail={`${product.quantity} units ordered`} value={money.format(product.revenue)}/>))}
                </LeaderCard>
                <LeaderCard icon={<CircleDollarSign className="h-5 w-5 text-primary"/>} title="Production queue" subtitle="Priority-weighted job lane">
                  {productionJobs.slice(0, 4).map((job, index) => (<LeaderRow key={job.id} index={index + 1} name={`${job.orders?.order_number || "Order"} - ${job.order_items?.products?.name || human(job.status)}`} detail={`${human(job.status)} - ${job.station || "No station"}`} value={job.priority ? `P${job.priority}` : "Live"}/>))}
                </LeaderCard>
              </section>
            </>)}
        </main>

        <OrderSheet data={dashboard} order={selectedOrder} open={Boolean(selectedOrder)} onOpenChange={(open) => !open && setSelectedOrder(null)} onRefresh={refresh}/>
      </div>
    </div>);
}
function StatCard({ label, value, hint }: {
    ...
    return (<Card className="min-h-[96px]">
...
function LeaderCard({ icon, title, subtitle, children }: {
    icon: ReactNode;
    title: string;
    subtitle: string;
    children: ReactNode;
...
function LeaderRow({ index, name, detail, value }: {
    index: number;
    name: string;
    detail: string;
    value: string;
}) {
    return (<div className="flex items-center gap-3 border-b py-3 last:border-0">
...
function OrderSheet({ data, order, open, onOpenChange, onRefresh, }: {
    ...
    return (<Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto sm:max-w-[60rem]">
        ...
        {order && (<div className="mt-6 space-y-6">
            ...
            <Section title="Line items" count={items.length}>
              {items.map((item) => (<div key={item.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div>
            ...
            <Section title="Production jobs" count={jobs.length}>
              {jobs.map((job) => (<div key={job.id} className="rounded-lg border p-3">
                ...
                <div className="rounded-lg border bg-secondary/30 p-3">
                  <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                    ...
                    <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                      ...
                      <SelectContent>
                        {productionStatuses.map((status) => (<SelectItem key={status} value={status}>
                            {human(status)}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
            ...
            <Section title="Payments and unread messages" count={payments.length + messages.length}>
              {payments.map((payment) => (<div key={payment.id} className="rounded-lg border p-3">
              ...
              {messages.map((message) => (<div key={message.id} className="rounded-lg border p-3">
