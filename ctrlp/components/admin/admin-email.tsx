// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-email.tsx
"use client";
import { Accordion,AccordionContent,AccordionItem,AccordionTrigger } from "@/ctrlp/components/ui/accordion";
...
import { AlignCenter,AlignLeft,AlignRight,Bell,Bot,Check,ChevronDown,Clock,Copy,Edit2,Eye,FileText,GripVertical,Image,Layout,LayoutList,Mail,MessageSquare,Minus,MousePointer,Phone,Plus,RefreshCw,Send,Settings,ShoppingBag,ToggleLeft,ToggleRight,Trash2,Type,Users,X,Zap } from "lucide-react";
...
const HISTORY_TYPE_META: Record<string, {
    label: string;
    color: string;
    icon: React.ElementType;
}> = {
...
async function adminGet<T>(path: string): Promise<T> {
    ...
    const res = await sourceFetch(path, { headers: { authorization: `Bearer ${token}` } });
...
function BlockPreview({ block, selected, onClick }: {
    ...
    switch (block.type) {
        case "header":
            inner = (<div style={{ background: bg, padding: `${pt}px ${ps}px ${pb}px`, textAlign: "center" }}>
        ...
        case "heading": {
            ...
            inner = (<div style={{ background: bg, padding: `${pt}px ${ps}px ${pb}px`, textAlign: p.align as "left" | "center" | "right" ?? "left" }}>
        ...
        case "text":
            ...
            break;
        case "button":
            inner = (<div style={{ background: bg, padding: `${pt}px ${ps}px ${pb}px`, textAlign: p.align as "left" | "center" | "right" ?? "center" }}>
        ...
        case "image":
          ...
          {p.src ? <img src={p.src} alt={p.alt} style={{ maxWidth: "100%", display: "block", margin: "0 auto" }}/> : <div style={{ height: 80, background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", color: "#aaa", fontSize: 13 }}>Image placeholder</div>}
            ...
            break;
        case "columns":
            inner = (<div style={{ background: bg, padding: `${pt}px ${ps}px ${pb}px`, display: "flex", gap: 16 }}>
        ...
        case "spacer":
            ...
            break;
        case "footer":
            inner = (<div style={{ background: bg, padding: `${pt}px ${ps}px ${pb}px`, textAlign: "center", fontSize: 12, color: p.color, lineHeight: 1.8 }}>
...
function BlockSettings({ block, onChange }: {
    block: EmailBlock;
    onChange: (props: Partial<EmailBlockProps>) => void;
}) {
    const p = block.props;
    ...
    function Field({ label, propKey, type = "text" }: {
        label: string;
        propKey: keyof EmailBlockProps;
        type?: string;
    }) {
        return (<div>
        <span className={labelClass}>{label}</span>
        <Input type={type} className={inputClass} value={String(p[propKey] ?? "")} onChange={(e) => onChange({ [propKey]: type === "number" ? Number(e.target.value) : e.target.value } as Partial<EmailBlockProps>)}/>
      </div>);
    }
    function ColorField({ label, propKey }: {
        label: string;
        propKey: keyof EmailBlockProps;
    }) {
        return (<div>
    ...
    function AlignField() {
        ...
        <div className="flex gap-1">
          {(["left", "center", "right"] as const).map((a) => (<Button key={a} variant="outline" size="icon" className={cn("h-7 w-7", p.align === a && "border-primary bg-primary/10")} onClick={() => onChange({ align: a })}>
...
export function AdminEmail() {
    ...
    useEffect(() => {
        ...
        if (tab === "submissions") {
            ...
            }>("/api/ctrlp/admin/communications/submissions")
        ...
        if (tab === "inbox") {
            ...
            }>("/api/ctrlp/admin/communications/inbox")
        ...
        if (tab === "automations") {
            ...
            }>("/api/ctrlp/admin/communications/automations")
        ...
        if (tab === "history") {
            ...
            adminGet<{
                events: HistoryEvent[];
            }>("/api/ctrlp/admin/communications/history")
    ...
    function switchEditorMode(mode: "visual" | "html") {
        if (mode === "html")
            setHtmlContent(blocksToFullHtml(editorBlocks));
        if (mode === "visual")
            setEditorBlocks([]);
        setEditorMode(mode);
    }
    ...
    async function sendEmail() {
        ...
        try {
            ...
            const res = await sourceFetch("/api/ctrlp/admin/communications/email", {
                method: "POST",
                headers: { "content-type": "application/json", ...(token ? { authorization: `Bearer ${token}` } : {}) },
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex gap-0.5 overflow-x-auto border-b scrollbar-none -mx-1 px-1">
        {EMAIL_TABS.map(([id, label]) => (<button key={id} onClick={() => setTab(id)} className={cn("shrink-0 border-b-2 px-3 py-2 text-[13px] font-medium transition-colors -mb-px", tab === id ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground")}>
            {label}
          </button>))}
      </div>
      ...
      {tab === "stats" && (<div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            ...
            ].map((h) => (<div key={h.label} className="rounded-lg border p-3">
            ...
            <Card>
              <CardContent className="p-4">
                ...
                {[["Due or queued", "0"], ["Journey emails sent", "0"], ["Total journey events", "0"]].map(([k, v]) => (<div key={k} className="flex items-center justify-between border-b py-2 last:border-0 text-sm">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                      {["Recipient", "Step", "Subject", "Status", "Attempted", "Scheduled"].map((h) => (<th key={h} className="px-3 py-2 text-left">{h}</th>))}
                    </tr>
                  </thead>
                  <tbody>
      ...
      {tab === "templates" && (<div className="space-y-4">
          ...
          {loadingTemplates ? (<div className="py-12 text-center text-muted-foreground text-sm"><Zap className="mx-auto mb-2 h-5 w-5 animate-pulse"/>Loading templates…</div>) : templates.length === 0 ? (<Card><CardContent className="py-16 text-center">
            ...
            </CardContent></Card>) : (<div className="overflow-hidden rounded-xl border bg-card">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {["Template Name", "Subject", "Category", "Status", "Updated", "Actions"].map((h) => (<th key={h} className="px-4 py-2.5 text-left">{h}</th>))}
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {templates.map((tmpl) => (<tr key={tmpl.id} className="hover:bg-muted/20">
      ...
      {/* ══ TEMPLATE EDITOR TAB ══ */}
      {tab === "editor" && (<div className="-mx-5 -mb-5 flex flex-col" style={{ height: "calc(100vh - 48px - 49px - 44px)" }}>
          {/* Editor header */}
          <div className="shrink-0 border-b bg-card px-4 py-2.5 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-1">
              ...
              {MERGE_TAGS.map((tag) => (<button key={tag} onClick={() => copyMergeTag(tag)} className={cn("flex items-center gap-1 rounded border px-1.5 py-0.5 text-[11px] font-mono transition-colors hover:border-primary/60 hover:bg-primary/5", copiedTag === tag && "border-green-500 bg-green-500/10 text-green-700")}>
                  {copiedTag === tag ? <Check className="h-2.5 w-2.5"/> : <Copy className="h-2.5 w-2.5 text-muted-foreground"/>}
                  {tag}
                </button>))}
            </div>

            {templateForm.subject && (<div className="text-[11px] text-muted-foreground">
          ...
          {/* Three-panel editor */}
          {editorMode === "visual" ? (<div className="flex flex-1 overflow-hidden">
              {/* Left: block library */}
              <div className="w-40 shrink-0 overflow-y-auto border-r bg-card/60 p-2">
                ...
                {BLOCK_LIBRARY.map(({ type, label, desc, icon: Icon }) => (<button key={type} onClick={() => addBlock(type)} className="mb-1 flex w-full items-start gap-2 rounded-lg border border-transparent px-2 py-2 text-left hover:border-border hover:bg-accent transition-colors">
              ...
              <div className="flex-1 overflow-y-auto bg-[#e9e9e9] p-6">
                {editorBlocks.length === 0 ? (<div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
                  ...
                  </div>) : (<div className="mx-auto max-w-[640px] overflow-hidden rounded shadow-sm">
                    {editorBlocks.map((block, i) => (<div key={block.id} draggable onDragStart={() => setDragIdx(i)} onDragOver={(e) => { e.preventDefault(); setDropIdx(i); }} onDragEnd={() => { setDragIdx(null); setDropIdx(null); }} onDrop={() => { if (dragIdx !== null)
                        moveBlock(dragIdx, i); setDragIdx(null); setDropIdx(null); }} className={cn("group relative", dropIdx === i && dragIdx !== i && "outline outline-2 outline-primary")}>
              ...
              <Textarea value={htmlContent} onChange={(e) => setHtmlContent(e.target.value)} className="h-full min-h-full font-mono text-xs leading-relaxed resize-none" placeholder="<!DOCTYPE html><html>...</html>"/>
            </div>)}
        </div>)}

      {/* ══ AUTOMATIONS TAB ══ */}
      {tab === "automations" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
          ...
          {/* Create / edit panel */}
          {autoForm !== null && (<Card className="border-primary/40">
              <CardContent className="p-5 space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  {autoForm.action_type === "send_sms" && (<div className="sm:col-span-2">
                ...
                <div className="flex items-center gap-2 pt-2 border-t">
                  <Button size="sm" disabled={autoSaving} onClick={async () => {
                    if (!autoForm.name || !autoForm.trigger_type || !autoForm.action_type) {
                    ...
                    try {
                        ...
                        await adminPost("/api/ctrlp/admin/communications/automations", autoForm as Record<string, unknown>, method);
          ...
          {/* Automations list */}
          {autoLoading ? (<div className="py-8 text-center text-sm text-muted-foreground"><Zap className="mx-auto mb-2 h-5 w-5 animate-pulse"/>Loading…</div>) : (<div className="overflow-hidden rounded-xl border bg-card">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                ...
                <tbody className="divide-y">
                  {automations.map((a) => (<tr key={a.id} className="hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2.5">
                        ...
                        await adminPost("/api/ctrlp/admin/communications/automations", { id: a.id, enabled: !a.enabled }, "PATCH");
                      ...
                      <td className="px-4 py-2.5">
                        ...
                        if (!confirm("Delete this automation?"))
                            return;
                        await adminPost("/api/ctrlp/admin/communications/automations", { id: a.id }, "DELETE");
          ...
          <div className="rounded-xl border bg-muted/20 p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {TRIGGER_TYPES.map(({ value, label, desc, icon: Icon }) => (<div key={value} className="flex items-start gap-2 rounded-lg border bg-card p-2.5">
      ...
      {/* ══ INBOX TAB ══ */}
      {tab === "inbox" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
          ...
          {inboxLoading ? (<div className="py-8 text-center text-sm text-muted-foreground"><Zap className="mx-auto mb-2 h-5 w-5 animate-pulse"/>Loading inbox…</div>) : inboxMessages.length === 0 ? (<Card>
            ...
            </Card>) : (<div className="flex gap-4 overflow-hidden rounded-xl border bg-card">
              {/* Message list */}
              <div className="w-64 shrink-0 divide-y border-r overflow-y-auto max-h-[500px]">
                {inboxMessages.map((m) => (<button key={m.id} onClick={() => {
                        setSelectedInbox(m);
                        if (!m.is_read) {
                            adminPost("/api/ctrlp/admin/communications/inbox", { id: m.id, is_read: true }, "PATCH");
      ...
      {tab === "submissions" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {(["all", "new", "read", "replied", "archived"] as const).map((s) => (<button key={s} onClick={() => setSubFilter(s)} className={cn("rounded-full border px-3 py-1 text-[12px] font-medium capitalize transition-all", subFilter === s ? "border-primary/60 bg-primary/10 text-foreground" : "border-transparent bg-muted text-muted-foreground hover:border-border")}>
                {s === "all"
          ...
          {subLoading ? (<div className="py-8 text-center text-sm text-muted-foreground"><Zap className="mx-auto mb-2 h-5 w-5 animate-pulse"/>Loading…</div>) : (<div className="space-y-2">
              {submissions
                    ...
                    .map((s) => (<div key={s.id} className={cn("rounded-xl border bg-card overflow-hidden", expandedSub === s.id && "border-primary/40")}>
                    {/* Header row */}
                    <button onClick={() => {
                        setExpandedSub(expandedSub === s.id ? null : s.id);
                        if (s.status === "new") {
                            adminPost("/api/ctrlp/admin/communications/submissions", { id: s.id, status: "read" }, "PATCH");
                      ...
                      <div className={cn("shrink-0 flex h-8 w-8 items-center justify-center rounded-full text-[12px] font-bold", s.status === "new" ? "bg-blue-500 text-white" : "bg-muted text-muted-foreground")}>
                    ...
                    {/* Expanded content */}
                    {expandedSub === s.id && (<div className="border-t px-4 py-4 space-y-3">
                        <p className="text-sm leading-relaxed whitespace-pre-wrap">{s.message}</p>
                        {s.notes && (<div className="rounded-lg bg-amber-500/10 border border-amber-500/20 px-3 py-2 text-sm">
                        ...
                        <div className="flex flex-wrap items-center gap-2 border-t pt-3">
                          ...
                          {(["read", "replied", "archived"] as const).map((status) => (status !== s.status && (<Button key={status} variant="outline" size="sm" className="h-7 text-[12px] capitalize" onClick={async () => {
                                await adminPost("/api/ctrlp/admin/communications/submissions", { id: s.id, status }, "PATCH");
              ...
              {submissions.filter((s) => subFilter === "all" || s.status === subFilter).length === 0 && (<Card><CardContent className="py-12 text-center">
      ...
      {tab === "history" && (<div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" className="gap-1.5" onClick={() => {
                ...
                adminGet<{
                    events: HistoryEvent[];
                }>(`/api/admin/communications/history${qs ? `?${qs}` : ""}`)
                    .then((r) => { setHistoryEvents(r.events ?? []); setHistoryLoading(false); });
            }}><RefreshCw className="h-3.5 w-3.5"/>Refresh</Button>
          </div>
          ...
          {historyLoading ? (<div className="py-8 text-center text-sm text-muted-foreground"><Zap className="mx-auto mb-2 h-5 w-5 animate-pulse"/>Loading history…</div>) : historyEvents.length === 0 ? (<Card><CardContent className="py-12 text-center">
            ...
            </CardContent></Card>) : (<div className="relative space-y-0">
              {/* Timeline line */}
              <div className="absolute left-[19px] top-0 bottom-0 w-px bg-border"/>
              {historyEvents.map((ev) => {
                    const meta = HISTORY_TYPE_META[ev.type] ?? HISTORY_TYPE_META.message;
                    const Icon = meta.icon;
                    return (<div key={ev.id} className="relative flex gap-4 pb-4 pl-12">
                    ...
                    <div className="flex-1 rounded-xl border bg-card p-3 hover:border-primary/30 transition-colors">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {ev.contact_email && (<div className="mt-0.5 flex items-center gap-1 text-[11px] text-muted-foreground">
      ...
      {/* ══ WIZARD TAB ══ */}
      {tab === "wizard" && (<div className="max-w-2xl space-y-4">
              ...
              <AccordionContent className="px-4 pb-4 border-t">
                <div className="pt-3 space-y-3 text-sm">
                  ...
                  <div className="grid gap-1.5">
                    {automations.slice(0, 4).map((a) => (<div key={a.id} className="flex items-center justify-between rounded-lg border bg-card/60 px-3 py-1.5">
