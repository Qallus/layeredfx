// Installer scheduling adapts the source service-professional directory and weekly availability.
// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-installers.tsx
"use client";
...
// ─── Installer form ────────────────────────────────────────────────────────────
function InstallerForm({ initial, onSave, onCancel, saving, }: {
    initial?: Installer | null;
    onSave: (data: Partial<Installer>) => void;
    onCancel: () => void;
    saving: boolean;
}) {
    const [name, setName] = useState(initial?.name ?? "");
    const [title, setTitle] = useState(initial?.title ?? "Graphic Installer");
    ...
    function handleSubmit() {
        ...
        onSave({
            name: name.trim(),
            title: title.trim() || "Graphic Installer",
            bio: bio.trim() || null,
    ...
    return (<div className="space-y-5 p-1">
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Senior Graphic Installer"/>
        </div>
        <div>
      ...
      <div className="flex gap-2 pt-1">
        <Button className="flex-1" disabled={saving || !name.trim()} onClick={handleSubmit}>
          {saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Saving…</> : "Save installer"}
        </Button>
...
export function AdminInstallers() {
    ...
    const [bookings, setBookings] = useState<(InstallerBooking & {
        installer_profiles?: {
            name: string;
        };
    })[]>([]);
    ...
    async function load() {
        ...
        try {
            const [dRes, bRes] = await Promise.all([
                adminFetch("/api/ctrlp/admin/installers?all=true"),
                sourceFetch("/api/ctrlp/installer-bookings?limit=100").then((r) => r.json()),
            ]);
            setInstallers(dRes.installers ?? []);
    ...
    useEffect(() => { load(); }, []);
    async function handleSave(data: Partial<Installer>) {
        setSaving(true);
        setMessage("");
        try {
            if (editing) {
                await adminFetch("/api/ctrlp/admin/installers", {
                    method: "PATCH",
                    body: JSON.stringify({ id: editing.id, ...data }),
                });
                setMessage("Installer updated.");
            }
            else {
                await adminFetch("/api/ctrlp/admin/installers", {
                    method: "POST",
    ...
    async function handleDelete(installer: Installer) {
        if (!window.confirm(`Deactivate "${installer.name}"? They won't appear to new customers.`))
            return;
        try {
            await adminFetch(`/api/admin/installers?id=${encodeURIComponent(installer.id)}`, { method: "DELETE" });
            setMessage("Installer deactivated.");
            await load();
        }
        catch (e) {
    ...
    const filteredInstallers = useMemo(() => {
        ...
        return installers.filter((d) => d.name.toLowerCase().includes(q) ||
            d.title.toLowerCase().includes(q) ||
            d.specialties.some((s) => s.toLowerCase().includes(q)));
    }, [installers, search]);
    const stats = useMemo(() => {
        const active = installers.filter((d) => d.is_active).length;
        ...
        return [
            { label: "Active installers", value: String(active) },
        ...
        <main className="px-4 py-5  ">
          ...
          {authState === "denied" && (<Card className="border-red-500/30">
              <CardContent className="p-5">
                ...
                <Button className="mt-4" asChild><a href="/login?redirect=/admin/installers">Go to login</a></Button>
              </CardContent>
            </Card>)}
          ...
          {authState === "allowed" && (<>
              ...
              <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                  <h1 className="text-[25px] font-semibold tracking-tight">Installers</h1>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Manage your installer roster, hourly rates, availability, and session bookings.
                  </p>
                </div>
              ...
              <div className="mb-5 flex gap-1 border-b">
                {(["roster", "bookings"] as const).map((v) => (<button key={v} onClick={() => setView(v)} className={cn("px-4 py-2 text-sm font-medium capitalize border-b-2 -mb-px transition-colors", view === v
                    ...
                    {v === "roster" ? "Installer Roster" : "Session Bookings"}
              ...
              {!loading && view === "roster" && (<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {filteredInstallers.length === 0 && (<div className="col-span-full py-10 text-center text-sm text-muted-foreground">
                      No installers yet.{" "}
                      <button onClick={openCreate} className="underline hover:text-foreground">
                        Add your first installer
                      </button>
                    </div>)}
                  {filteredInstallers.map((d) => (<Card key={d.id} className={cn(!d.is_active && "opacity-60")}>
                      <CardContent className="p-5">
                      ...
                      <CardContent className="p-4">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            ...
                            {b.installer_profiles && (<div className="mt-0.5 text-xs text-muted-foreground">
                                Installer: <span className="font-medium text-foreground">{b.installer_profiles.name}</span>
                              </div>)}
                          </div>
                          <div className="shrink-0 text-right">
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
          <SheetHeader className="mb-5">
            <SheetTitle>{editing ? `Edit — ${editing.name}` : "Add installer"}</SheetTitle>
          </SheetHeader>
          <InstallerForm initial={editing} onSave={handleSave} onCancel={() => { setSheetOpen(false); setEditing(null); }} saving={saving}/>
        </SheetContent>
      </Sheet>
    </div>);
}
