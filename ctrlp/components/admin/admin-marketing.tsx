// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-marketing.tsx
"use client";
...
async function adminToken() {
    ...
    if (!session?.access_token)
        throw new Error("Sign in again before sending a campaign.");
...
function CampaignSheet({ open, onOpenChange, users, onSent }: {
    open: boolean;
    ...
    const [subject, setSubject] = useState("LayeredFX update");
    ...
    async function send() {
        ...
        try {
            ...
            const response = await sourceFetch("/api/ctrlp/admin/messaging/send", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ channel, mode: "bulk", role, userIds: targetUsers.map((user) => user.id), subject, body }) });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok)
                throw new Error(payload.error || "Could not send campaign.");
...
function Segment({ title, count, detail }: {
    title: string;
    count: number;
    detail: string;
}) {
...
function FieldSelect({ label, value, onChange, items }: {
    label: string;
    value: string;
