...
import { getCurrentAdminProfile,loadAdminDashboardData,markMessageRead } from "@/ctrlp/lib/admin/admin-api";
...
import { Archive,BookOpen,Calendar,CalendarPlus,ChevronLeft,ChevronRight,Inbox,List,Mail,MessageSquare,Pencil,Phone,Plus,RefreshCw,Search,Send,Table2,Trash2,User } from "lucide-react";
...
export function AdminMessages() {
    ...
    async function deleteTemplate(id: string) {
        try {
            ...
            const res = await sourceFetch(`/api/admin/booking-templates?id=${encodeURIComponent(id)}`, {
                method: "DELETE",
    ...
    async function syncEmailInbox() {
        ...
        try {
            const token = await getAdminToken();
            const response = await sourceFetch("/api/ctrlp/admin/messaging/sync-email", {
                method: "POST",
                headers: { authorization: `Bearer ${token}` },
            });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok)
                throw new Error(payload.error || "Could not sync email.");
    ...
    const visibleMessages = useMemo(() => {
        ...
        if (!needle)
            return messages;
        return messages.filter((m) => [m.subject, m.body, m.channel, m.direction, m.order_id].some((v) => String(v || "").toLowerCase().includes(needle)));
    }, [messages, query]);
    const visibleContacts = useMemo(() => {
        const needle = contactQuery.trim().toLowerCase();
        if (!needle)
            return contactSubmissions;
        return contactSubmissions.filter((s) => [s.first_name, s.last_name, s.email, s.phone, s.company, s.subject, s.message].some((v) => String(v || "").toLowerCase().includes(needle)));
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="px-4 py-5  ">
          ...
          {authState === "allowed" && (<>
              ...
              <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                ...
                <div className="flex gap-2">
                  {activeSection === "inbox" ? (<>
                    ...
                    </>) : activeSection === "contact" ? (<Button variant="outline" onClick={refreshContacts}>
                      <RefreshCw className="h-4 w-4"/> Refresh
                    </Button>) : (<Button onClick={() => { setEditingTemplate(null); setTemplateFormOpen(true); }}>
              ...
              <div className="mb-5 flex border-b border-border">
                <button onClick={() => setActiveSection("inbox")} className={cn("flex items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium -mb-px transition-colors", activeSection === "inbox" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground")}>
                  <Inbox className="h-4 w-4"/> Messages
                </button>
                <button onClick={() => setActiveSection("contact")} className={cn("flex items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium -mb-px transition-colors", activeSection === "contact" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground")}>
                  <User className="h-4 w-4"/> Contact Page
                  {contactNew > 0 && (<Badge className="h-4 bg-primary/20 px-1.5 text-[10px] text-foreground">{contactNew}</Badge>)}
                </button>
                <button onClick={() => setActiveSection("templates")} className={cn("flex items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium -mb-px transition-colors", activeSection === "templates" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground")}>
                  <Calendar className="h-4 w-4"/> Booking Templates
                </button>
              </div>

              {notice && (<div className="mb-4 rounded-lg border bg-secondary/40 px-3 py-2 text-sm text-muted-foreground">{notice}</div>)}
                    ...
                    <Card>
                      ...
                      <CardContent className="p-0">
                        <Table>
                          ...
                          <TableBody>
                            {visibleMessages.map((message) => (<TableRow key={message.id} className="cursor-pointer hover:bg-accent/45" tabIndex={0} onClick={() => openMessage(message)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    openMessage(message);
                } }}>
                            ...
                            {!visibleMessages.length && (<TableRow><TableCell className="p-6 text-center text-muted-foreground" colSpan={5}>No messages found.</TableCell></TableRow>)}
                          </TableBody>
                        </Table>
                      </CardContent>
                    </Card>
              ...
              {activeSection === "contact" && (<>
                  ...
                  <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="relative sm:w-72">
                      ...
                      <Input className="h-8 pl-9 text-xs" placeholder="Search name, email, subject..." value={contactQuery} onChange={(e) => setContactQuery(e.target.value)}/>
                    </div>
                    <div className="flex items-center gap-0.5 rounded-lg border bg-secondary/40 p-1 w-fit">
                      {(["table", "list", "calendar"] as const).map((v) => (<button key={v} onClick={() => setContactView(v)} className={cn("flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium capitalize transition-colors", contactView === v
                  ...
                  {contactView === "table" && (<ContactTableView submissions={visibleContacts} onSelect={openContact}/>)}
                  {contactView === "list" && (<ContactListView submissions={visibleContacts} onSelect={openContact}/>)}
                  {contactView === "calendar" && (<ContactCalendarView submissions={visibleContacts} month={contactCalendarMonth} onMonthChange={setContactCalendarMonth} selectedDay={selectedCalendarDay} onDaySelect={setSelectedCalendarDay} onSelect={openContact}/>)}
                </>)}
                          ...
                          {templates.map((template) => (<TableRow key={template.id}>
                              ...
                              <TableCell>
                                <div className="flex items-center gap-1">
                                  <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Edit template" onClick={() => { setEditingTemplate(template); setTemplateFormOpen(true); }}>
                                    <Pencil className="h-3.5 w-3.5"/>
                                  </Button>
                                  {deletingTemplateId === template.id ? (<div className="flex items-center gap-1">
                                    ...
                                    </div>) : (<Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" aria-label="Delete template" onClick={() => setDeletingTemplateId(template.id)}>
                                      <Trash2 className="h-3.5 w-3.5"/>
                                    </Button>)}
                                </div>
                              </TableCell>
                            </TableRow>))}
                          {!templates.length && (<TableRow>
        ...
        {/* Sheets */}
        <BookingTemplateSheet open={templateFormOpen} onOpenChange={setTemplateFormOpen} template={editingTemplate} onSaved={(saved) => {
        ...
        <ComposeMessageSheet open={composeOpen} onOpenChange={(open) => { setComposeOpen(open); if (!open)
        setComposePrefill(null); }} orders={orders} users={users} config={config} prefill={composePrefill} onMessageSent={refreshData}/>
        <MessageDetailSheet message={selectedMessage} open={Boolean(selectedMessage)} onOpenChange={(open) => { if (!open)
        setSelectedMessageId(null); }} order={orders.find((o) => o.id === selectedMessage?.order_id) ?? null} user={users.find((u) => u.id === selectedMessage?.user_id) ?? null} onCompose={() => { setSelectedMessageId(null); setComposeOpen(true); }}/>
        <ContactSubmissionSheet submission={selectedContact} open={Boolean(selectedContact)} onOpenChange={(open) => { if (!open)
        setSelectedContactId(null); }} onStatusChange={handleContactStatusChange} onCompose={(prefill) => {
...
async function getAdminToken() {
    ...
    if (!token)
        throw new Error("Sign in again before using messaging.");
    return token;
}
async function loadMessagingConfig(): Promise<MessagingConfig | null> {
    ...
    if (!session)
        return null;
    const response = await sourceFetch("/api/ctrlp/admin/messaging/config", {
...
async function loadBookingTemplates(): Promise<BookingTemplate[]> {
    try {
        const token = await getAdminToken();
        const response = await sourceFetch("/api/ctrlp/admin/booking-templates", {
            headers: { authorization: `Bearer ${token}` },
        });
        if (!response.ok)
            return [];
...
async function loadContactSubmissions(): Promise<ContactSubmission[]> {
    try {
        const token = await getAdminToken();
        const response = await sourceFetch("/api/ctrlp/admin/contact-submissions", {
            headers: { authorization: `Bearer ${token}` },
        });
        if (!response.ok)
            return [];
...
async function updateContactStatus(id: string, status: string) {
    const token = await getAdminToken();
    await sourceFetch("/api/ctrlp/admin/contact-submissions", {
        method: "PATCH",
...
// ─── Shared UI atoms ──────────────────────────────────────────────────────────
function MessageStat({ label, value, hint }: {
    label: string;
    value: string;
    hint: string;
}) {
...
function ChannelCard({ icon, title, ready, rows }: {
    icon: ReactNode;
    title: string;
    ready: boolean;
    rows: [
    ...
    return (<Card>
      ...
      <CardContent className="space-y-2">
        {rows.map(([label, value]) => (<div key={label} className="flex items-center justify-between rounded-lg border bg-background/35 px-3 py-2 text-sm">
...
function TemplateCard({ title, items }: {
    title: string;
    items: string[];
}) {
    return (<Card>
...
function MessageMeta({ label, value, subvalue }: {
    label: string;
    value: string;
    subvalue?: string;
}) {
...
function ContactTableView({ submissions, onSelect }: {
    ...
    return (<Card>
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {submissions.map((s) => (<TableRow key={s.id} className="cursor-pointer hover:bg-accent/45" tabIndex={0} onClick={() => onSelect(s)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            onSelect(s);
        } }}>
                <TableCell className="pl-4">
...
function ContactListView({ submissions, onSelect }: {
    submissions: ContactSubmission[];
    onSelect: (s: ContactSubmission) => void;
}) {
    ...
    return (<div className="space-y-3">
      {submissions.map((s) => (<Card key={s.id} className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => onSelect(s)}>
          <CardContent className="p-4">
...
function ContactCalendarView({ submissions, month, onMonthChange, selectedDay, onDaySelect, onSelect, }: {
    submissions: ContactSubmission[];
    month: Date;
    ...
    return (<div className="grid gap-6 lg:grid-cols-[300px_1fr]">
      <Card className="h-fit">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <button onClick={() => onMonthChange(new Date(year, monthNum - 1, 1))} className="grid h-7 w-7 place-items-center rounded-md hover:bg-accent">
        ...
        <CardContent>
          <div className="mb-1 grid grid-cols-7">
            {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (<div key={d} className="py-1 text-center text-[10px] font-medium text-muted-foreground">{d}</div>))}
          </div>
          <div className="grid grid-cols-7 gap-0.5">
            ...
            return (<button key={key} onClick={() => onDaySelect(isSelected ? null : key)} className={cn("relative flex aspect-square flex-col items-center justify-center rounded-md text-xs transition-colors", isSelected ? "bg-primary text-primary-foreground font-semibold" : "hover:bg-accent", isToday && !isSelected && "font-bold text-primary", count > 0 && !isSelected && "bg-primary/8")}>
                  <span>{day}</span>
                  {count > 0 && (<span className={cn("absolute bottom-0 text-[8px] font-bold leading-tight", isSelected ? "text-primary-foreground/80" : "text-primary")}>{count}</span>)}
                </button>);
        })}
          </div>
          <div className="mt-3 border-t pt-3 text-xs text-muted-foreground">
            {selectedDay ? (<div className="flex items-center justify-between">
                <span>
              ...
              </div>) : (<span className="block text-center">Click a day to filter</span>)}
          </div>
        </CardContent>
      </Card>
...
function ContactSubmissionSheet({ submission, open, onOpenChange, onStatusChange, onCompose, }: {
      ...
      <SheetContent className="overflow-y-auto sm:max-w-[64rem]">
        ...
        <div className="mt-5">
          ...
          <div className="flex flex-wrap gap-2">
            {submission.phone && (<Button variant="outline" size="sm" asChild>
                <a href={`tel:${submission.phone}`}>
                  <Phone className="mr-1.5 h-3.5 w-3.5"/>Call
                </a>
              </Button>)}
            {submission.phone && (<Button variant="outline" size="sm" onClick={() => { onOpenChange(false); onCompose({ channel: "sms", recipient: submission.phone! }); }}>
        ...
        {/* Subject */}
        {submission.subject && (<div className="mt-4">
        ...
        <div className="mt-5">
          ...
          <div className="flex flex-wrap gap-2">
            {STATUS_OPTIONS.map((opt) => (<button key={opt.value} onClick={() => onStatusChange(submission.id, opt.value)} className={cn("rounded-md border px-3 py-1.5 text-xs font-medium transition-colors", submission.status === opt.value
                ? opt.color
...
function MessageDetailSheet({ message, open, onOpenChange, order, user, onCompose, }: {
      ...
      <SheetContent className="overflow-y-auto sm:max-w-[60rem]">
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <MessageMeta label="Customer" value={user?.full_name || user?.email || order?.customer_email || "Not linked"} subvalue={user?.company || order?.company || user?.phone || order?.customer_phone || undefined}/>
            <MessageMeta label="Order" value={order?.order_number ? `#${order.order_number}` : message.order_id ? message.order_id : "Not linked"} subvalue={order ? `${human(order.status)} / ${human(order.payment_status)}` : undefined}/>
          </div>
          ...
          <div className="flex gap-2">
            ...
            {order && (<Button variant="outline" asChild><Link href="/admin/orders">Open orders</Link></Button>)}
            <Button variant="outline" onClick={() => onOpenChange(false)}>Close</Button>
          </div>
        </div>
      </SheetContent>
...
function ComposeMessageSheet({ open, onOpenChange, orders, users, config, prefill, onMessageSent, }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    ...
    async function sendMessage() {
        ...
        try {
            const token = await getAdminToken();
            const response = await sourceFetch("/api/ctrlp/admin/messaging/send", {
                method: "POST",
                headers: { "content-type": "application/json", authorization: `Bearer ${token}` },
                body: JSON.stringify({
                    channel,
      ...
      <SheetContent className="overflow-y-auto sm:max-w-[60rem]">
        ...
        <div className="mt-6 space-y-4">
          ...
          {mode === "single" && (<div>
              ...
              <Input value={target === "manual" ? recipient : resolvedRecipient} onChange={(e) => setRecipient(e.target.value)} disabled={target !== "manual"} placeholder={channel === "sms" ? "+14805551212" : "customer@example.com"}/>
            </div>)}
          {mode === "bulk" && (<div className="rounded-lg border bg-secondary/30 p-3 text-sm text-muted-foreground">
          ...
          {target !== "manual" && selectedUser && mode === "single" && (<div className="rounded-lg border bg-secondary/30 p-3 text-sm text-muted-foreground">
          ...
          {channel === "sms" && (<div className="rounded-lg border bg-secondary/30 p-3 text-xs text-muted-foreground">
              SMS replies should point Twilio Messaging webhook to {config?.twilio?.webhookUrl || "/api/ctrlp/webhooks/twilio/sms"}.
            </div>)}
          {channel === "email" && (<div className="rounded-lg border bg-secondary/30 p-3 text-xs text-muted-foreground">
...
function BookingTemplateSheet({ open, onOpenChange, template, onSaved, }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
      ...
      <SheetContent className="overflow-y-auto sm:max-w-[56rem]">
        ...
        <div className="mt-6 space-y-5">
          ...
          <div>
            ...
            <Textarea value={body} onChange={(e) => setBody(e.target.value)} className="min-h-[180px]" placeholder={channel === "sms"
            ? "LayeredFX: Your {{typeName}} is confirmed for {{date}} at {{time}}. Reply STOP to opt out." : "Hi {{firstName}},\n\nYour {{typeName}} appointment is confirmed for {{date}} at {{time}}.\n\nSee you soon!"}/>
