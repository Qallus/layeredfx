// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-orders.tsx
"use client";
...
import { createAdminOrder,getCurrentAdminProfile,loadAdminDashboardData,updateAdminOrder } from "@/ctrlp/lib/admin/admin-api";
...
function statusTone(status: string) {
    if (["paid", "completed", "ready", "delivered", "proof_approved", "approved"].includes(status))
        return "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300";
    if (["pending", "awaiting_payment", "proof_pending", "printing", "finishing", "file_check", "print_ready", "in_production", "proofing"].includes(status))
        return "border-primary/20 bg-primary/15 text-lime-800 dark:text-lime-200";
    if (["unpaid", "failed", "cancelled", "refunded", "on_hold", "design_needed"].includes(status))
...
export function AdminOrders() {
    ...
    async function refresh(openOrderId?: string) {
        ...
        setData(next);
        if (openOrderId)
            setSelectedOrder(next.orders.find((order) => order.id === openOrderId) ?? null);
    }
...
function CreateOrderSheet({ open, onOpenChange, data, onCreated }: {
    open: boolean;
    ...
    async function validateCoupon() {
        ...
        try {
            ...
            const res = await sourceFetch("/api/ctrlp/checkout/validate-coupon", {
                method: "POST",
            ...
            if (!res.ok) {
                ...
                setCouponResult({ ...data.coupon, discount_amount: data.discount_amount });
                setMessage(`Coupon applied: saves ${money.format(data.discount_amount)}`);
            }
        }
        catch {
...
function OrderDrawer({ order, data, open, onOpenChange, onRefresh }: {
    order: Order | null;
    data: AdminDashboardData | null;
    open: boolean;
    onOpenChange: (open: boolean) => void;
