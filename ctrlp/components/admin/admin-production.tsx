// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-production.tsx
"use client";
...
import { createProductionJob,getCurrentAdminProfile,loadAdminDashboardData,updateProductionJob } from "@/ctrlp/lib/admin/admin-api";
import type { AdminDashboardData,ProductionJob } from "@/ctrlp/lib/admin/types";
...
function statusTone(status: string) {
    if (["completed", "ready", "proof_approved"].includes(status))
        return "border-emerald-500/20 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300";
    if (["file_check", "proof_pending", "print_ready", "printing", "finishing", "install_scheduled"].includes(status))
        return "border-primary/20 bg-primary/15 text-lime-800 dark:text-lime-200";
    if (["design_needed", "on_hold"].includes(status))
...
export function AdminProduction() {
    ...
    async function refresh(openJobId?: string) {
        ...
        setData(next);
        if (openJobId)
            setSelectedJob(next.productionJobs.find((job) => job.id === openJobId) ?? null);
    }
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="px-4 py-5  ">
          ...
          {authState === "denied" && (<Card className="border-red-500/30"><CardContent className="p-5"><div className="font-semibold text-red-600 dark:text-red-300">Admin access required</div><Button className="mt-4" asChild><a href="/login?redirect=/admin/production">Go to login</a></Button></CardContent></Card>)}
          {authState === "allowed" && (<>
              ...
              <Card>
                ...
                const item = orderItems.find((line) => line.id === job.order_item_id);
                const assignee = users.find((user) => user.id === job.assigned_staff_id);
                return (<TableRow key={job.id} className="cursor-pointer hover:bg-accent/45" onClick={() => setSelectedJob(job)}>
        ...
        <ProductionJobSheet open={createOpen} onOpenChange={setCreateOpen} data={data} staff={staff} onSave={async (input) => {
            const job = await createProductionJob(input);
            await refresh(job.id);
        }}/>
        <ProductionJobSheet open={Boolean(selectedJob)} onOpenChange={(open) => {
            if (!open)
                setSelectedJob(null);
        }} job={selectedJob} data={data} staff={staff} onSave={async (input) => {
            if (!selectedJob)
                return;
...
function ProductionStat({ label, value, hint }: {
    label: string;
    value: string;
    hint: string;
}) {
...
function ProductionJobSheet({ open, onOpenChange, job, data, staff, onSave, }: {
    open: boolean;
