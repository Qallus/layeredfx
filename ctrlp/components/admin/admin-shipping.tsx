// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-shipping.tsx
"use client";
...
import { getCurrentAdminProfile,loadAdminDashboardData,previewShippingRate,saveAdminShipment } from "@/ctrlp/lib/admin/admin-api";
import type { AdminDashboardData,Shipment } from "@/ctrlp/lib/admin/types";
import { getSupabaseBrowserClient } from "@/ctrlp/lib/supabase/browser";
import { cn } from "@/ctrlp/lib/utils";
import { Calculator,Plus,Send } from "lucide-react";
...
function statusTone(status: string | null | undefined) {
    ...
    if (["shipped", "in_transit", "out_for_delivery", "ready_to_ship"].includes(status || ""))
        return "border-primary/20 bg-primary/15 text-lime-800 dark:text-lime-200";
    if (["exception", "returned"].includes(status || ""))
...
export function AdminShipping() {
    ...
    async function refresh(openShipmentId?: string) {
        ...
        setData(next);
        if (openShipmentId)
            setSelectedShipment(next.shipments.find((shipment) => shipment.id === openShipmentId) ?? null);
    }
    ...
    return (<div className="source-module">
      <div className="min-h-full bg-background text-foreground">
        ...
        <ShipmentSheet open={createOpen || Boolean(selectedShipment)} onOpenChange={(open) => { if (!open) {
        setCreateOpen(false);
        setSelectedShipment(null);
    } }} shipment={selectedShipment} data={data} onSaved={refresh}/>
        <RateSheet open={quoteOpen} onOpenChange={setQuoteOpen}/>
      </div>
    </div>);
}
function ShippingStat({ label, value, hint }: {
    label: string;
    value: string;
    hint: string;
}) {
...
function ShipmentSheet({ open, onOpenChange, shipment, data, onSaved }: {
    open: boolean;
...
function RateSheet({ open, onOpenChange }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}) {
    const [carrier, setCarrier] = useState("ups");
