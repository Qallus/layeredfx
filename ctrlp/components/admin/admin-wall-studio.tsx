// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/admin-wall-studio.tsx
"use client";
...
import { WS_CATEGORY_LABEL,type WsCategory } from "@/ctrlp/lib/wall-studio/types";
import { sourceFetch } from '@/lib/dashboard/source-runtime';
import { LayoutTemplate,Pencil,Save } from "lucide-react";
...
function DesignForm({ initial, onSave, onCancel }: {
    initial: Design;
    onSave: (d: Partial<Design>) => Promise<void>;
    onCancel: () => void;
}) {
...
export function AdminWallStudio() {
    ...
    async function load() {
        setLoading(true);
        try {
            const data = await apiFetch("/api/ctrlp/admin/wall-studio");
            setDesigns(data.products ?? []);
            const rows: RuleRow[] = data.rules ?? [];
            const map = new Map(rows.map((r) => [r.key, r.value]));
            const rv: Record<string, string> = {};
            for (const f of RULE_FIELDS)
                rv[f.key] = toDisplay(f.kind, map.get(f.key));
            setRuleValues(rv);
    ...
    async function saveDesign(update: Partial<Design>) {
        if (!editing)
            return;
        await apiFetch("/api/ctrlp/admin/wall-studio", {
            method: "PATCH",
    ...
    async function saveRules() {
        ...
        setNotice("");
        try {
            const patches = RULE_FIELDS.map((f) => apiFetch("/api/ctrlp/admin/wall-studio", {
                method: "PATCH",
                body: JSON.stringify({ target: "rule", key: f.key, value: fromDisplay(f.kind, ruleValues[f.key] ?? "0") }),
            }));
            patches.push(apiFetch("/api/ctrlp/admin/wall-studio", {
                method: "PATCH",
                body: JSON.stringify({
                    target: "rule",
      ...
      <div className="min-h-full bg-background text-foreground">
        ...
        <main className="px-4 py-5  ">
          ...
          {authState === "allowed" && (<>
              ...
              {loading ? (<Card><CardContent className="p-5 text-sm text-muted-foreground">Loading…</CardContent></Card>) : (<div className="space-y-5">
                  {/* Designs */}
                  <Card>
                    ...
                    <CardContent className="p-0">
                      <Table>
                        ...
                        <TableBody>
                          {designs.map((d) => (<TableRow key={d.id}>
                    ...
                    <CardContent className="space-y-5">
                      <div>
                        ...
                        <div className="grid gap-3 sm:grid-cols-3">
                          {BASE_RATE_CATS.map((c) => (<div key={c}>
                      ...
                      <div>
                        ...
                        <div className="grid gap-3 sm:grid-cols-3">
                          {RULE_FIELDS.map((f) => (<div key={f.key}>
        ...
        <Sheet open={sheetOpen} onOpenChange={(open) => { setSheetOpen(open); if (!open)
        setEditing(null); }}>
