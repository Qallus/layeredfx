// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/import-products-sheet.tsx
"use client";
...
type MappedField = "name" | "sku" | "slug" | "category" | "short_description" | "description" | "tagline" | "product_type" | "base_price" | "sale_price" | "base_cost" | "vendor" | "status" | "stock_status" | "featured" | "customizer_enabled" | "video_url" | "dimension_width_in" | "dimension_height_in" | "dimension_length_in" | "weight_lbs" | "shipping_class" | "tax_status" | "photo_gallery" | "gallery" | "template_files" | "quantity_tiers" | "sizes" | "materials" | "print_options" | "finishing_options" | "turnaround_times" | "tags" | "woo_product_id";
type ImportResult = {
    row: number;
    sku: string;
    name: string;
...
export function ImportProductsSheet({ open, onClose, onImported }: Props) {
    ...
    async function runImport() {
        ...
        try {
            ...
            if (!token)
                throw new Error("Sign in again before importing.");
            const mapped = applyMapping(parsedRows, mapping);
            const response = await sourceFetch("/api/ctrlp/admin/products/import", {
                method: "POST",
                headers: {
    ...
    return (<Sheet open={open} onOpenChange={(o) => !o && handleClose()}>
      <SheetContent className="overflow-y-auto sm:max-w-[56rem]">
        ...
        <div className="mt-6 space-y-5">
          ...
          {step === "upload" && (<div className="space-y-4">
              <div className={cn("flex flex-col items-center justify-center rounded-lg border-2 border-dashed px-6 py-12 text-center transition-colors", dragging ? "border-primary bg-primary/5" : "border-border hover:border-primary/50")} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
          ...
          {step === "preview" && (<div className="space-y-4">
              ...
              {!requiredMapped && (<div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-300">
              ...
              {/* Column mapping */}
              <div>
                <button className="flex w-full items-center gap-2 rounded-lg border bg-secondary/30 px-3 py-2 text-sm font-medium hover:bg-secondary/60" onClick={() => setShowMapping(!showMapping)}>
                ...
                {showMapping && (<div className="mt-2 rounded-lg border bg-background p-3">
                    <div className="grid gap-2 sm:grid-cols-2">
                      {ALL_FIELDS.map((field) => (<div key={field} className="flex items-center gap-2">
              ...
              <div className="overflow-x-auto rounded-lg border">
                ...
                {parsedRows.length > 10 && (<div className="border-t px-3 py-2 text-xs text-muted-foreground">
          ...
          {/* ── STEP: Importing ── */}
          {step === "importing" && (<div className="flex flex-col items-center gap-4 py-12 text-center">
          ...
          {step === "done" && (<div className="space-y-4">
            ...
            ].map(({ label, value, color }) => (<div key={label} className="rounded-lg border bg-secondary/30 p-3 text-center">
                    <div className={cn("text-2xl font-semibold", color)}>{value}</div>
                    <div className="mt-1 text-xs text-muted-foreground">{label}</div>
                  </div>))}
              </div>

              {results.length > 0 && (<div className="max-h-[320px] overflow-y-auto rounded-lg border">
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {results.map((r) => (<TableRow key={r.row}>
                          ...
                          <TableCell>
                            {r.skipped ? (<span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><CheckCircle2 className="h-3.5 w-3.5"/> Skipped</span>) : r.success ? (<span className="inline-flex items-center gap-1 text-xs text-green-600 dark:text-green-400"><CheckCircle2 className="h-3.5 w-3.5"/> OK</span>) : (<span className="inline-flex items-center gap-1 text-xs text-red-600 dark:text-red-400"><AlertCircle className="h-3.5 w-3.5"/> {r.error}</span>)}
                          </TableCell>
                        </TableRow>))}
                    </TableBody>
                  </Table>
