// Adapted from CTRL+P 015a7b58b80e63ef87c73bec549a23242b88f3e3: components/admin/import-users-sheet.tsx
"use client";
...
export function ImportUsersSheet({ open, onClose, onImported }: Props) {
    ...
    async function runImport() {
        ...
        try {
            ...
            const resp = await sourceFetch("/api/ctrlp/admin/users/import", {
                method: "POST",
                headers: {
    ...
    return (<Sheet open={open} onOpenChange={(v) => { if (!v)
        handleClose(); }}>
      <SheetContent className="w-full sm:max-w-2xl flex flex-col gap-0 p-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {step === "upload" && (<div className="space-y-4">
              <button onClick={downloadSampleCSV} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                <Download className="h-4 w-4"/>
                Download sample CSV template
              </button>

              <div className={cn("flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-12 text-center transition-colors cursor-pointer", dragging ? "border-primary bg-primary/5" : "border-border hover:border-primary/50")} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={handleDrop} onClick={() => fileRef.current?.click()}>
          ...
          {step === "preview" && (<div className="space-y-5">
              ...
              <div className="rounded-lg border overflow-hidden">
                <button className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold bg-muted/40 hover:bg-muted/70 transition-colors" onClick={() => setShowMapping((p) => !p)}>
                  Column mapping
                  {showMapping ? <ChevronDown className="h-4 w-4"/> : <ChevronRight className="h-4 w-4"/>}
                </button>
                {showMapping && (<div className="p-4 grid grid-cols-2 gap-3 text-sm">
                    {(Object.keys(FIELD_LABELS) as (keyof MappedRow)[]).map((field) => (<div key={field}>
                        <label className="block text-xs text-muted-foreground mb-1">{FIELD_LABELS[field]}</label>
                        <Select value={mapping[field] ?? "__none__"} onValueChange={(v) => setMapping((prev) => ({ ...prev, [field]: v === "__none__" ? undefined : v }))}>
              ...
              <div>
                ...
                .map((f) => <TableHead key={f} className="py-2 text-xs">{FIELD_LABELS[f]}</TableHead>)}
                    ...
                    <TableBody>
                      {applyMapping(previewRows, mapping).map((row, i) => (<TableRow key={i} className="text-xs">
                          {(["email", "full_name", "first_name", "last_name", "phone", "company", "role"] as (keyof MappedRow)[])
                    .filter((f) => mapping[f])
                    .map((f) => (<TableCell key={f} className="py-1.5 max-w-[150px] truncate">
                ...
                {parsedRows.length > 5 && (<p className="text-xs text-muted-foreground mt-1.5">…and {parsedRows.length - 5} more rows</p>)}
              </div>
              ...
              <div className="rounded-lg border p-4 space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={defaultRole} onValueChange={setDefaultRole}>
                    ...
                    <SelectContent>
                      {editableRoles.map((r) => (<SelectItem key={r} value={r}>{r.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
          ...
          {/* ── Step: Importing ── */}
          {step === "importing" && (<div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
          ...
          {step === "done" && (<div className="space-y-5">
              ...
              {summary.placeholders > 0 && (<div className="rounded-lg border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/30 p-4">
              ...
              {summary.failed > 0 && (<div className="rounded-lg border overflow-hidden">
                  <button className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-950/50 transition-colors" onClick={() => setShowErrors((p) => !p)}>
                  ...
                  {showErrors && (<div className="divide-y text-xs">
                      {results.filter((r) => !r.success && !r.skipped).map((r) => (<div key={r.row} className="px-4 py-2.5 flex items-start gap-3">
              ...
              {summary.succeeded > 0 && (<div className="rounded-lg border overflow-hidden">
                  ...
                  <div className="divide-y text-xs max-h-52 overflow-y-auto">
                    {results.filter((r) => r.success).map((r) => (<div key={r.row} className="px-4 py-2 flex items-center gap-3">
                        <span className="font-mono text-muted-foreground w-8 shrink-0">#{r.row}</span>
              ...
              {summary.skipped > 0 && (<div className="rounded-lg border p-4">
