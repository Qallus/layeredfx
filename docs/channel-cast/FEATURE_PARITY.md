...
`/studio` provides perspective photo visualization, sample finishes/custom graphics, foreground brush/polygon masks, undo/redo, before/after, device-local saved look and PNG export. Automatic point-prompted foreground selection is now available through browser-local SlimSAM, with a review step, layer visibility and manual refinement. It does not create paid orders. `/book` now provides source-style appointment cards, calendar, individual preferred times and retained customer details; availability-backed reservation and notifications remain unconnected.

## Navigation, Inspiration and advanced Wall Studio (2026-09-11)

Services has a responsive mega menu and ten dedicated pages; About Us and Inspiration are separate pages. Navbar consultation links go to /book; estimate triggers go to /studio. The existing dashboard blog editor saves drafts, publishes, schedules, archives and deletes stories consumed by the public archive and individual routes. Local development persists blog JSON on the computer in .local-data (shared across local browsers, not browser-only); production uses a separate server-only Supabase table with verified admin membership and optimistic revision checks. The blog schema is a review draft and has not been applied to live Supabase.

Wall Studio adds first-use downloaded SlimSAM object selection in a worker, foreground mask review/visibility, zoom, material rotation, six Wilsonart woodgrain swatches with source/sample links, and multi-wall quantity/cost estimates with user-entered rates. No vendor prices, availability or automatic photo measurements are claimed. Selection may need brush/outline refinement. Photos stay on device; model downloads require internet. Estimates exclude preparation, taxes, shipping and minimums. Real phone-camera behavior and live blog database remain staging/device checks.
