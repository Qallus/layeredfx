...
Review `supabase/migrations/20260910_layeredfx_operations.sql` against that target. It creates only `lfx_ops_members` and `lfx_ops_store`. Execute it only after confirming the project identity and backups.

Applied 2026-09-14 via Supabase MCP to the "Layered FX" project (`tdywcdgbavfcsywejoka`, us-east-1), which had an empty `public` schema and no prior migrations. Recorded as migration version `20260914203954` (`layeredfx_operations`). Verified afterwards: both tables exist with RLS enabled and all check/PK/FK constraints; `anon` and `authenticated` have no table grants; `service_role` has access. Follow-up migration `20260914204906` (`layeredfx_ops_hardening`, mirrored in `supabase/migrations/20260914_layeredfx_ops_hardening.sql`) added `lfx_ops_members_user_id_idx` and revoked RPC execution of the platform `rls_auto_enable()` event-trigger function from `public`/`anon`/`authenticated`; the security advisor no longer reports it. Initial admin member `jw@layeredfx.com` was inserted into organization `1e9fd71c-0404-4095-87ed-976a87bc8020`. The `lfx_ops_store` row is seeded by the server on first read. On 2026-09-14 the blog, top bar, coupons, portals, contact submissions and business cards schemas were applied from their reviewed drafts as migrations `layeredfx_blog`, `layeredfx_topbar`, `layeredfx_coupons`, `layeredfx_portals`, `layeredfx_contact_submissions` and `layeredfx_business_cards` (versions `20260914210113`–`20260914210144`). The applied SQL is mirrored in `supabase/migrations/20260914_layeredfx_*.sql`; those files, not the drafts, are authoritative. Differences from the drafts: no `begin`/`commit` (the MCP wraps each migration in a transaction), revision and array checks on `lfx_topbar_store`, and covering indexes for `lfx_coupons.changed_by`, `lfx_business_cards.changed_by` and `lfx_portal_accounts.user_id`. The single-table `lfx_business_card_store` draft under `.review/integration` is superseded and was not applied. Verified afterwards: all ten `lfx_*` tables have RLS enabled and no `anon`/`authenticated`/`PUBLIC` grants; buckets `lfx-portal-media` (private, 20 MB, image/video) and `lfx-card-media` (public, 8 MB, images) exist under the 50 MB project limit; advisors report only the intentional no-policy notices, unused indexes on empty tables and disabled leaked-password protection.
