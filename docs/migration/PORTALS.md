...
Live activation requires the existing isolated `LFX_SUPABASE_*` and `LFX_OPERATIONS_ORG_ID` variables. The schema was applied to the LayeredFX Supabase project on 2026-09-14 as `supabase/migrations/20260914_layeredfx_portals.sql` (from `sql/portals-review-draft.sql`). It enables RLS and revokes direct anonymous/authenticated access, granting only service-role table access. Verified 2026-09-14: anon table requests return 401, and `pg_policies` has no policies on the `storage` schema, so no broad policy grants direct access to the private `lfx-portal-media` bucket.

Configure Supabase email confirmation and SMTP, the site URL and allowlisted `/login` redirect before testing registration. Users confirm their email and then sign in with their password. An interrupted signup between Auth and account creation needs staff reconciliation; it is reported as incomplete, not silently successful. Password reset, OAuth/social login, magic links, organizational subusers and public business-card hosting are not part of this increment. The LayeredFX project is configured and a read-only API check passed, but live registration, login, media upload and staff approval have not yet been exercised end to end. `.env.local` now sets `LFX_PORTAL_MODE=supabase`, so local development uses live Auth and will send real confirmation emails.
