...
## Wall Studio duplicate step navigation removal
- Severity: low. File: components/layeredfx/wall-studio.tsx. Evidence: numbered horizontal ws-guide repeated the same seven destinations already available in the tool rail.
- Fix: removed the duplicate row and its unused CSS; retained tool rail and contextual Back/Continue guidance. Original homepage unchanged.
- Verification: npm install, npm test, npm run typecheck, npm run lint and npm run build passed in the separate review worktree. Exact output in docs/reviews/logs/*-verified.txt. Existing lint warnings remain.
- A concurrent root build failed collecting /admin/blog while the development checkout was active; isolated review build passed. No production integrations changed.

## Wall Studio desktop and mobile redesign
- Severity: medium usability. Files: components/layeredfx/wall-studio.tsx, components/layeredfx/studio.css. Evidence: properties previously sat between the rail and canvas and consumed the mobile workspace. Reference reviewed: docs/wall-studio/Wall Studio UI redesign/Wall Studio 2.dc.html and github.md.
- Fix: dedicated studio header, left tool rail, central canvas, right properties panel with persistent Back/Continue actions. Mobile uses the same rail horizontally and a collapsible/swipeable bottom properties drawer. Existing seven steps, material catalog, estimation inputs, patterns and graphics remain available. Theme toggle and public footer retained.
- Selection: grow/shrink edges apply one-pixel alpha dilation/erosion to the combined selection. Clear all is a history operation, so Undo and Redo can restore it. Brush add, erase, outline and tap selection remain available. Selection history allows individual visibility and removal.
- Review: keyboard-accessible before/after comparison slider, wall and selection summary, local save, PNG download and native file sharing where supported; unsupported browsers download the image to share manually. No public hosted sharing link is implied.
- Verification: browser checks exercise actual brush canvas edits, grow/shrink/clear and pixel-exact undo; desktop panel positions, live material application, review slider, mobile drawer and overflow. Evidence: logs/studio-redesign.json and screenshots/studio-redesign-desktop.png, studio-redesign-mobile.png. Unit tests cover morphology boundaries and non-cascading growth. Full required command output is in logs/*-verified.txt and verification.json.
- Remaining limits: automatic selection continues to use the existing on-device model; this increment tests refinement without repeating model download tests. Native phone camera/share sheets need physical-device confirmation. Material and installation rates are entered by the user; the reference's illustrative prices were not copied. Homepage implementation unchanged.
- Final validation: npm install succeeded; 158 tests passed; typecheck, lint and production build exited 0. Existing lint warnings remain. Root homepage and updated /studio both opened successfully on port 3000.

## Studio estimate, intake, and direct graphic positioning
- Severity: medium. The selected material panel inherited an undefined/translucent background; its new full-column slide-up surface is explicitly opaque white with readable foreground colors.
- Estimate is now the final editor step. A contrasting summary above the details form expands with wall area, material, installation, rush, travel and preparation status. Captured fields include ground height, obstacles, pictures/paintings removal, holes, imperfections, old wraps/graphics, interior/exterior, address, contact identity and requested install date/time.
- Rush is one exclusive tier: <=24h $1,000; <=48h $500; <=7 days $200; <14 days $150. Travel is $10 per started 10 one-way road miles from the 85226 location returned by the geocoder. Missing routing is shown as unpriced, never zero.
- Travel uses configurable Nominatim-compatible and OSRM-compatible endpoints (LFX_STUDIO_GEOCODER_URL, LFX_STUDIO_ROUTER_URL). No service is guessed or auto-enabled. Public Nominatim is explicitly refused for private customer addresses. Provider information: https://operations.osmfoundation.org/policies/nominatim/ and https://project-osrm.org/docs/v5.5.1/api/ . Physical ZIP origin is approximate until a street origin is specified.
- Request persistence is a locked local development inbox, or authenticated portal intake into the existing operations store using an explicitly configured active staff/admin owner (LFX_STUDIO_LEAD_OWNER_ID). Production returns a setup/authentication error if unavailable. No live migration or production write performed.
- Wall Studio leads appear in /admin/leads with all submitted details. Add to Pipeline calls the standard lead conversion rules and preserves project snapshots. Convert to Job requires closed-won and the existing project handoff, preserving stage history. These studio jobs are shown above the existing /admin/jobs screen; the broader source dashboard job backend remains unintegrated.
- Schedule Consultation prefills the booking form and preserves the project in its message. It requests a time, not a guaranteed booking. Project Deposit saves a deposit request only: amount/percentage and payment provider configuration remain unspecified; no charge or checkout success is implied. Current material/labor rates remain entered values pending approved vendor pricing. Repairs/access are marked for site review, not assigned fabricated fees.
- Customize supports pointer dragging through inverse wall perspective plus numeric positioning and six alignment controls. Existing layer rendering and foreground masks remain intact.
- Verification: scripts/studio-estimate-browser.mjs passed opaque panel, canvas text drag/alignment, estimate submission, lead visibility, Add to Pipeline and blocked premature Job conversion. tests/studio-project.test.mjs passed deadline/travel boundaries, idempotent conversion, field retention, viewer denial and won-only job conversion. See logs/studio-estimate-flow.json. Physical routing and payment cannot be tested without configuration.
- Final checks for this increment: npm install, 160 tests, typecheck, lint and production build all passed (existing lint warnings). The isolated browser flow passed. Root files have been synchronized for local review.

## Centered object-selection progress toast
- Severity: medium usability. File: components/layeredfx/wall-studio.tsx. Selection progress was buried in the right properties panel and easily missed.
- Fix: progress now appears immediately when the selection queue becomes nonempty in a centered, opaque toast with an edge-finding heading, live selection count, first-use download status and cancel control. The toast does not intercept photo taps except its cancel button; it disappears when the queue finishes or is cancelled. The duplicate properties-panel progress block is removed.
- Verification: desktop browser with a deterministic worker stub checked centered position, queue growth from one to two through the toast, no progress block in properties and cancellation dismissal. Screenshot: screenshots/studio-selection-toast.png. The model implementation is unchanged; no new model inference claim is made. Required command output is recorded in logs/*-verified.txt.
- Final validation: npm install, 160 tests, typecheck, lint and production build passed. Existing lint warnings remain.

## Wall Specs: click-to-draw graphic area
- Severity: medium usability. Added a second Wall Specs tool alongside Corner handles. Click to draw accepts ordered polygon points, Undo point, Cancel drawing, Finish area, or tapping the first point to close. Unfinished/invalid outlines cannot advance to Materials.
- Drawn outlines show a live filled/dashed preview and a closed marching-ants boundary. Concave areas are supported; crossing, degenerate or out-of-bounds polygons are rejected. The material uses the polygon bounding rectangle for texture layout and is clipped to the exact outline before applying foreground protection. Existing measured width/height fields remain unchanged.
- Shape is included in saved looks and used by preview, download and share rendering. Choosing Corner handles returns to the rectangular perspective workflow; changing the room resets the drawn shape.
- Verification: scripts/studio-draw-browser.mjs passed adding five points, Undo point, closing, unchanged pixels outside the applied area, and saving/reloading the same outline. Evidence: screenshots/studio-drawn-area.png. Unit tests cover concave/reversed polygons and invalid/crossing geometry. Full command logs: logs/*-verified.txt.
- Final required validation passed: npm install, 162 tests, typecheck, lint and production build. Existing lint warnings remain.

## First-batch selection toast and subsequent progress card
- Severity: low usability. The centered toast previously reappeared for every queue.
- Fix: keep it until the first complete queue drains; later queues show progress in a contrasting, sticky Object Detection card in the properties column. The card stays visible between selections. Starting a new room resets the introduction; restoring a look with automatic masks uses the card. Cancelling an unfinished initial batch does not count as completion.
- Verification: controlled-worker browser check queued two initial taps, completed the first while retaining the toast, completed the second to show the card, then verified a new tap updates only the card. Checked solid background and cancellation. Screenshot: screenshots/studio-selection-progress-card.png. Selection model unchanged.
- Required checks passed: npm install, 162 tests, typecheck, lint and production build. Existing lint warnings remain.

## Sample furniture and material adjustments
- Severity: low usability. Material sliders were native blue ranges in the properties panel, separate from the preview.
- Fix: components/layeredfx/studio-material-controls.tsx reuses the existing shadcn/Radix Slider. Pattern Scale, Rotation and Blend Strength now appear directly below the central canvas with visible values, themed tracks/thumbs and keyboard focus. Removed duplicate property-panel ranges. Controls appear after material application, including Patterns and Customize.
- Scene assets: replaced the sample selector/default with room-residential-v2.png (tree removed, coffee table added) and room-commercial-v2.png (tree removed, reception desk added). Visually inspected generated edits. Versioned paths preserve existing saved looks and their masks against their original photos.
- Verification: npm install exited 0; npm test passed all 162 tests; npm run typecheck, npm run lint and npm run build exited 0. Exact output is in logs/*-verified.txt. Existing lint warnings remain.
- Browser: scripts/studio-material-controls-browser.mjs passed updated scene path, keyboard scale increment, absence of duplicate sliders in properties, desktop placement, mobile controls above the collapsed drawer with no horizontal overflow, and original homepage heading. Screenshots: studio-material-controls-desktop.png and studio-material-controls-mobile.png.
- Remaining limitations: expanded mobile properties cover part of the preview by design; collapse the drawer to adjust the canvas controls. Previously saved looks retain their original room image. No AI segmentation model changes or new production service verification in this UI update.

## Resizable Wall Studio canvas
- Severity: low usability. Fixed desktop column widths limited the working canvas area.
- Fix: added the shadcn Resizable composition using react-resizable-panels v4.12.4, with a visible branded separator between canvas and properties. Canvas defaults to 72%; properties have a 300px minimum. Pointer and keyboard resizing preserve the active editor and selections. Mobile retains the existing bottom drawer and hides the desktop separator.
- Files: components/layeredfx/ui/resizable.tsx, wall-studio.tsx, studio.css, package.json and package-lock.json.
- Verification: npm install passed; 162 tests, typecheck, lint and production build passed. Exact outputs: logs/*-verified.txt. Existing lint warnings remain. scripts/studio-resizable-browser.mjs passed pointer drag, keyboard arrows, mobile drawer/no overflow and original homepage checks. Screenshots: studio-resizable-desktop.png and studio-resizable-mobile.png.
- Sync safety: automatic review initially rejected copying root files because unrelated local changes had not been ruled out. Exact transformation and manifest/CSS diff checks proved only this update differed; the subsequent sync was approved. No unrelated changes replaced.
- Remaining limitations: desktop split resets on page reload; no resize control is presented on mobile.

## Shared compact frontend navigation in Wall Studio
- Severity: medium usability. Wall Studio omitted the frontend menu; Services used custom details-based desktop behavior.
- Fix: shared Header uses shadcn Navigation Menu composition on the existing Radix stack, with compact links, hover/click and keyboard support. Preserved three Services columns, no service arrows, and a full-width CTA. Studio now renders Header with the app icon, account, theme and consultation actions; Save look/Share moved into the canvas toolbar. Studio omits the promotional top bar to preserve editor height. Mobile uses the shared navigation sheet.
- Files: components/layeredfx/header.tsx, navigation.css, ui/navigation-menu.tsx, wall-studio.tsx and dependency manifests.
- Browser verification: scripts/frontend-navigation-menu-browser.mjs passed homepage, Contact and Studio hover/Enter/Escape, three columns, studio icon and editor progression, mobile Contact navigation and no page overflow. Visually checked screenshots/navigation-studio.png; corrected initial content width so the CTA and three columns fit comfortably. Initial browser attempt during dependency recompilation timed out; subsequent runs passed.
- Required verification: npm install, 162 tests, typecheck, lint and production build; exact logs in logs/*-verified.txt. Existing lint warnings remain. No auth, data or production-service changes.

## Remove menu dropdown arrows
- Severity: low visual preference. Removed the remaining ChevronDown icon from My Account in the shared frontend header (including Wall Studio). Services already has no arrow; native summary markers remain hidden.
- File: components/layeredfx/header.tsx. Dropdown behavior retained; browser verified Login opens and summary contains no SVG.
- Validation: npm install, 162 tests, typecheck, lint and production build passed. Exact outputs in logs/*-verified.txt; existing lint warnings remain. No remaining blockers for this change.

## Customer message scroller
- Severity: low usability. Dashboard Messages used an unbounded full-message body in its detail sheet. No separate customer-facing chat widget was found in the current repository; asked for clarification and used the existing customer message reader as the implementation target.
- Added official @shadcn/react Message Scroller wrappers in components/layeredfx/ui/message-scroller.tsx with branded CSS, bounded viewport, accessible region/log, and Jump to latest. Integrated into ctrlp/components/admin/admin-messages.tsx. The reader opens at the start of the selected message and resets when its ID changes. Existing transport, permissions and reply/forward actions remain unchanged.
- Validation: npm install passed; 162 tests, typecheck, lint and build passed. Exact output: logs/*-verified.txt. Existing lint warnings remain.
- Browser: temporary isolated 30-message fixture rendered the actual component and Jump to latest moved scrollTop to 970; keyboard Home exercised. Initial new-route request timed out before recompilation, reload passed. Temporary fixture removed. Existing local Messages dataset is empty, so no live customer conversation or delivery was tested.
- Remaining limitation: this integrates the existing dashboard message reader, not a newly built public chat service. The reusable component is ready for a separate chat widget once its intended location is identified.

## Viewport studio, material picker and wall-first selection
- Severity: medium usability. Editor controls could scroll toward the footer and material confirmation could fall below long result lists.
- Fix: viewport-fixed desktop/mobile workspace, independently scrolling properties/image, bottom canvas sliders visible above even the expanded mobile drawer. Wider residential v3 image shows ceiling and recessed lights; old assets remain for saved-mask alignment. Zoom retains its scrollable enlarged image.
- Materials apply directly on card click with loading feedback, no Apply/Cancel panel. Searchable, themed Radix dialog library picker replaces the native select; material-family icons, keyboard focus and selected check marks included.
- Object Detection offers Select wall area. The existing segmentation output is complemented to protect all pixels outside the wall, including furniture. Each new wall selection atomically replaces the prior protection mask; undo restores prior edits. Brush/outline tools can refine imperfect detections. The selection persists through normal saved mask data; shared previews now also retain drawn wall clipping.
- Files: wall-studio.tsx, studio.css, wilsonart-library.tsx, material-family-picker.tsx, lib/studio/invert-runs.ts, render.ts, residential-v3 asset.
- Verification: npm install passed; 164 tests, typecheck, lint and production build passed; exact logs in logs/*-verified.txt. Browser scripts/studio-refine-browser.mjs passed wall-target response routing with a stub worker, family search, direct application and fixed desktop/mobile slider bounds, including dark picker. Screenshots: studio-fixed-desktop.png, studio-fixed-mobile.png, studio-picker-dark.png. Complement unit tests cover missing rows, split runs, full/empty masks. Existing lint warnings remain.
- Live detector limitation: actual SlimSAM test timed out at 120 seconds. Follow-up instrumented worker reported `Failed to fetch` and zero masks, triggering the existing fallback error. Automatic detection therefore remains unverified in this environment because its model fetch failed; no segmentation-quality claim is made. Existing brush/outline tools remain available. Screenshot studio-wall-detection-live.png records the failed live attempt.

## Graphics interaction, pagination and Estimate controls
- Severity: medium. Custom material rendering left a translated/rotated canvas transform active before graphics decoration, so drawn items did not match hit-test coordinates. Reset the transform in decorate; use measured, rotation-aware hit boxes. Added visible pointer/touch drag handles with keyboard arrows and native desktop palette-to-canvas drop for text/shapes/icons. New clicked items start higher on the wall. Corrected symbol encoding and rounded displayed position values.
- Materials pagination now has equal Previous/Next buttons on one row with no page count.
- Estimate uses shadcn-style Radix Toggle/Select and React DayPicker calendar in an accessible dialog, with custom hour/minute selects. Light/dark branded styling, Arizona date serialization and future-date validation retained. Existing text/number fields keep their branded styling and data bindings. No changes to rates or payment delivery.
- Verification: npm install, 165 tests, typecheck, lint and production build passed; exact logs in logs/*-verified.txt. New regression test checks transform reset before layer positioning. Browser scripts/studio-graphics-form-browser.mjs passed three graphic drags/keyboard moves, palette drop, pagination row, toggle state, Installation choice, date/hour persistence and mobile dark picker bounds. Screenshots studio-graphics-drag.png, studio-estimate-calendar.png, studio-estimate-mobile-dark.png.
- Local server restarted after dependency-install resolution cache errors; now serving port 3000 successfully. Existing lint warnings remain. Desktop drop and pointer handles tested; real-device touch hardware was not available. Existing external model-download limitation is unchanged.


## Homepage photographic design concepts
- Severity: low. Hero and service cards used flat illustrated scenes where photographic mockups were requested. Updated components/layeredfx/hero.tsx and services.tsx with four generated photographic design concepts matching the original warm living room, kitchen, privacy glass, and two-tone paint compositions. The hero and architectural finishes card share the room asset. Layout, service data, filters, dialogs, and links remain intact.
- Assets: public/images/architectural-room-photo.png, kitchen-photo.png, window-film-photo.png, paint-photo.png. Generated with the image-generation tool; source PNGs retained under the local .codex/generated_images/01a08d28-8ec8-7d62-9416-fce268c81a80 directory. These are design mockups, not completed customer project photographs. Updated descriptive alt text and hero concept label.
- Browser verification: hero and all four card images loaded; Film & paint filter returned two cards; service detail dialog opened; desktop 1440px and mobile 390px rendered without horizontal overflow. Visually inspected hero, card grid and mobile crop. Evidence: logs/home-hero-photo-desktop.png, home-services-photo-desktop.png, home-hero-photo-mobile.png.
- Required verification: npm install, npm test (165 passed, zero failed), npm run typecheck, npm run lint and npm run build all exited 0. Exact outputs retained in logs/*-verified.txt and verification.json. Existing lint warnings remain; no blockers for this image update.


## Hero actions, wallpaper and photographic comparison
- Severity: low. Updated hero actions to Customize Your Space (Palette icon) and Explore Our Services (PanelsTopLeft icon); kept both on one row, including 320px mobile. Existing studio route and services anchor preserved.
- Added botanical wallpaper photographic mockup to Make it tactile; renamed Set a new tone to Roman Clay Texture. Replaced inspiration card and dialog SVGs with matching photographic room, kitchen and privacy film concepts.
- Replaced comparison SVGs with generated aligned room photographs: plain white wall and framed art before; oak cladding and custom abstract vinyl mural after. Comparison slider and concept disclosure preserved. New PNG source generations retained locally; repo assets wallpaper-room-photo.png, room-before-photo.png, room-after-photo.png.
- Verification: npm install, 165 tests, typecheck, lint and production build passed. Fixed an initially missed icon import before final verification. Exact outputs: logs/*-verified.txt. Browser checked same-row hero bounds at 1440/390/320px without horizontal overflow; customization navigates to studio; services anchor/title, inspiration image loads, comparison image loads and keyboard slider passed. Initial browser check expected a dialog; corrected to verify the actual existing studio navigation. Visually reviewed comparison screenshot logs/home-comparison-photo.png. Existing lint warnings remain; no blockers for this scope.


## Six journal articles and editable email copies
- Severity: medium. Public journal had no launch articles and Email Template Editor used the disconnected source content endpoint. Added six original articles (Wall Wraps, Wallpaper, Wall Cladding, Wall Studio, Design Services, Roman Clay), each with distinct photographic mockup, category, metadata, public URL and studio/booking/contact links. Added responsive sticky article sidebar CTA. Images reuse the reviewed generated design assets and remain explicitly identified as mockups.
- Files: lib/blog/launch-posts.json, launch.ts, model.ts, server.ts, app/api/blog/manage/route.ts, app/inspiration/[slug]/page.tsx, frontend.css, admin-email.tsx. Initial empty storage receives six published articles and six linked draft email copies with full article content, subject, preheader, image and absolute links. Existing stores are preserved. Once saved, removals and edits persist; no reseeding by slug. Public pages exclude email templates regardless of status. Managed reads filter posts/templates. Existing admin authorization, origin/body limits and optimistic revisions remain. No schema migration or live database action.
- Both editors now use the same local/server content persistence boundary. Email copies are independent drafts after creation, not a continuous sync. Local editor saves verified; production database/provider integration remains unverified. No email sending was added or executed. Existing communications telephony initialization displays an unrelated missing-token error in demo mode; it does not prevent template editing/saving.
- Verification: six public article routes, six image loads, sidebar links, dashboard post list, six template rows, actual Blog editor and Email Template Editor save buttons, stale-save 409 and mobile overflow checks passed. Regression test covers template source/subject preservation, stale-save rejection, immutable type and public exclusion. Exact npm outputs in logs/*-verified.txt; screenshots journal-post-desktop.png and journal-email-editor.png. Existing lint warnings remain.
- Final required checks: npm install exited 0; npm test passed 166 tests; typecheck, lint and production build exited 0.


## Homepage copy cleanup and commercial film card
- Severity: low. Removed the three user-specified notes from Inspiration, Comparison and Material Studio. Wrap card now displays Wall wraps, Cabinet wraps and Appliance wraps, retaining its subtitle. Updated See light differently with a generated photographic commercial conference-room privacy-film mockup and descriptive alt text. Asset public/images/commercial-window-film-photo.png; source retained under .codex/generated_images.
- Verification: npm install, 166 tests, typecheck, lint and production build passed; exact logs in logs/*-verified.txt. Browser confirmed all three notes absent, exact three-item wrap list, commercial image load, and mobile layout without overflow. Initial browser assertion used a shell-corrupted middle dot; corrected the test escape and reran successfully. Screenshot logs/home-commercial-film.png. Existing lint warnings remain; no blockers.


## Blog sidebar button typography
- Severity: low. Scoped frontend.css override matches the three journal sidebar action links to navbar booking-button text: 11px, weight 500, shared brand foreground and hover color tokens.
- Browser verification: computed size, weight and color equal the navbar button for all three links in light and dark modes (11px, 500, rgb(194, 205, 197)).
- Required checks passed: npm install, 166 tests, typecheck, lint and production build. Exact output in logs/*-verified.txt. Existing lint warnings remain.


## Contact page, submission handoff and communication preferences
- Severity: high (fixed locally). Contact previously only staged a portal draft. New direct contact form persists normalized inquiries and up to three 2 MB JPEG/PNG/WebP/PDF attachments; validates signatures and bounded request size; staff-only download sets attachment, nosniff and sandbox headers. Contact Form Submission dashboard inbox includes original data, timestamps, flags, file links, Convert to Lead and Add to Pipeline.
- Lead/pipeline mutations retain original submission metadata and follow the existing command engine. Production resolves the saved submission by ID server-side. Repeat conversion does not duplicate identities; normal lead conversion also retains form data. Existing stage validation still gates project/job handoff.
- Contact UI follows supplied reference: centered introduction, form/map columns, branded custom select and consent toggles, drop/browse upload, call/text/email/booking cards. Office coordinates verified through OpenStreetMap: 33.4870643, -111.9243922. Embedded map loads with pulsing marker; reduced-motion disables animation. Initial restricted-browser map network failure resolved with verified OSM embed and network-enabled browser check.
- Footer includes supplied phone/text/email, exact 2026 Scottsdale copyright, Privacy, Terms, Opt-In and Opt-Out links. Four pages added with services/portal/upload terms, optional separate channel choices, STOP/HELP/rates/frequency language and consent records. Sources and deployment notes: contact-implementation.md.
- Browser checks passed: actual contact submission with PDF, staff download, inbox to lead to pipeline, opt-out, four legal/preference routes, mobile overflow, external map load. Screenshots contact-desktop.png (initial map blocked), contact-mobile.png, contact-map-verified.png. Removed the two fictional test submissions after checking; browser operations used an isolated context.
- Security/regression checks cover malformed and oversized public bodies, wrong origins, protected list/download, consent channel validation, file signatures/count, file metadata filtering, source preservation and repeat conversion/viewer rejection. Fixed two component imports and JSX apostrophe lint errors before final checks.
- Remaining production blockers: reviewed docs/migrations/20260912-contact-submissions.sql is not applied or staging-verified; no live Supabase mutation performed. Twilio/email delivery and provider suppression/STOP webhook integration are not enabled. Website preferences are recorded, not a claim of carrier registration or provider-side unsubscribe completion. Policies are drafts for business/legal review. Existing in-process rate limiting requires deployment-level abuse controls for production. Existing broader dashboard parity limits remain.
- Final required verification: npm install exited 0; 171 tests passed; npm run typecheck, npm run lint and npm run build exited 0. Exact output in logs/*-verified.txt; existing lint warnings remain.
