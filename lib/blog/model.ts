...
export function published(items:ContentItem[],now=Date.now()){return items.filter(p=>p.content_type==='blog_post'&&(p.status==='published'||p.status==='scheduled')&&!!p.published_at&&Date.parse(p.published_at)<=now).sort((a,b)=>Date.parse(b.published_at!)-Date.parse(a.published_at!));}
export function mutateBlog(state:BlogState,method:string,body:Record<string,unknown>):{state:BlogState;item?:ContentItem}{
 ...
 if(!['blog_post','email_template'].includes(String(body.content_type)))throw new OperationError('Only posts and email templates can be saved here.',400);
 if(previous&&previous.content_type!==body.content_type)throw new OperationError('Content type cannot be changed.',400);
 const email=body.content_type==='email_template';
 const title=text(body.title,180),slug=text(body.slug||(email?(previous?.slug||`email-${crypto.randomUUID()}`):''),180).toLowerCase();
