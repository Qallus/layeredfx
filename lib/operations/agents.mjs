...
export function agentCommand(s,c,actor,now){
 ...
 switch(c.type){
  ...
  case 'agent.assignment.result':{
   // Recorded by /api/agents/send after the provider answers. The operations route refuses this from clients.
   const item=(s.agentAssignments||[]).find(a=>a.id===c.id)||fail('Assignment not found.',404);
   if(!admin&&item.createdBy!==actor.id)fail('Only the person who assigned this or an administrator can send it.',403);
   const outcome=oneOf(c.outcome,['sent','failed'],'send outcome');
   const service=oneOf(c.service,['hermes','paperclip'],'agent service');
   const previous=item.delivery||{attempts:0};
   item.delivery={state:outcome,service,externalId:text(c.externalId,200),externalUrl:httpsUrl(c.externalUrl,'Agent link'),attempts:(previous.attempts||0)+1,lastAttemptAt:now,lastError:outcome==='failed'?text(c.detail,500):'',requestId:text(c.requestId,64)};
   // A failed send leaves the assignment queued so it can be sent again.
   if(outcome==='sent'&&item.status==='queued')item.status='in_progress';
   item.updatedAt=now;
   const entry=outcome==='sent'?`Sent to ${service}${item.delivery.externalId?` · ${item.delivery.externalId}`:''}`:`Send failed · ${text(c.detail,300)}`;
   item.log=[...(item.log||[]),{at:now,actorId:actor.id,actorName:actor.name,text:entry}].slice(-50);
   return item.id;
  }
