...
import {agentCommand} from './agents.mjs';
...
// Personal dashboard profile (My profile). Email and role always come from the signed-in LayeredFX account.
const MEMBER_PROFILE_FIELDS = { fullName: 200, jobTitle: 150, bio: 1000, phone: 60, company: 200, location: 150 };
const TIMEZONES = ['America/Phoenix', 'America/Los_Angeles', 'America/Denver', 'America/Chicago', 'America/New_York', 'America/Anchorage', 'Pacific/Honolulu', 'UTC'];
function memberProfilePatch(p) {
    const out = {};
    for (const [key, max] of Object.entries(MEMBER_PROFILE_FIELDS))
        if (key in p)
            out[key] = text(p[key], max);
    if ('avatarUrl' in p) {
        out.avatarUrl = text(p.avatarUrl, 1000);
        if (out.avatarUrl) {
            let url;
            try { url = new URL(out.avatarUrl); }
            catch { fail('Profile photo must be an https URL.'); }
            if (url.protocol !== 'https:' || url.username || url.password)
                fail('Profile photo must be an https URL.');
        }
    }
    if ('timezone' in p)
        out.timezone = oneOf(p.timezone, TIMEZONES, 'timezone');
    if ('appearance' in p)
        out.appearance = oneOf(p.appearance, ['light', 'dark'], 'appearance');
    return out;
}
...
export function visibleState(state, actor) {
    ...
    // Teammates see only a member's name, title and photo; the rest of a profile stays with its owner.
    out.memberProfiles = Object.fromEntries(Object.entries(out.memberProfiles || {}).map(([id, p]) => [id, id === actor.id ? p : { fullName: p.fullName || '', jobTitle: p.jobTitle || '', avatarUrl: p.avatarUrl || '' }]));
...
export function applyCommand(input, c, actor, at = new Date().toISOString()) {
    ...
    switch (c.type) {
        case 'profile.save': {
            // A member can only edit their own profile; any id in the command is ignored.
            s.memberProfiles ||= {};
            s.memberProfiles[actor.id] = { ...(s.memberProfiles[actor.id] || {}), ...memberProfilePatch(c.patch || {}), updatedAt: now };
            resultId = actor.id;
            break;
        }
        case 'team.save': {
        ...
        default:
            ...
            else if(c.type.startsWith('agent.'))resultId=agentCommand(s,c,actor,now);
