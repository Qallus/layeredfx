...
const TEAM_FIELDS = { name: 200, title: 150, department: 120, email: 254, phone: 60, tagline: 200, bio: 4000, availability: 200, photoUrl: 1000, secondaryPhotoUrl: 1000 };
function teamPatch(p) {
    ...
    for (const key of ['photoUrl', 'secondaryPhotoUrl'])
        if (out[key]) {
            let url;
            try { url = new URL(out[key]); }
            catch { fail('Photo links must be https URLs.'); }
            if (url.protocol !== 'https:' || url.username || url.password)
                fail('Photo links must be https URLs.');
        }
    if ('attributes' in p) {
    ...
    switch (c.type) {
        case 'team.save': {
            ...
            if (c.id) {
                ...
                if (!patch.name)
                    fail('Team member name is required.');
                const member = { id: uid('team'), status: 'active', attributes: [], sortOrder: s.team.reduce((max, m) => Math.max(max, m.sortOrder || 0), 0) + 1, createdAt: now, ...patch, updatedAt: now };
                s.team.push(member);
                resultId = member.id;
            }
            break;
        ...
        case 'team.move': {
            requireAdmin(actor);
            const ordered = [...(s.team || [])].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
