...
import {mutateNotice,activeNotices,type NoticeState} from './model';
const file=()=>path.join(process.cwd(),'.local-data','topbar.json');
async function database(query:string,init:RequestInit={}){
 if(!configured())throw new OperationError('Notification storage requires LayeredFX server configuration.',503);
 ...
 const response=await fetch(`${process.env.LFX_SUPABASE_URL}/rest/v1/lfx_topbar_store?org_id=eq.${org}${query}`,{...init,headers:{apikey:process.env.LFX_SUPABASE_SERVICE_ROLE_KEY!,Authorization:`Bearer ${process.env.LFX_SUPABASE_SERVICE_ROLE_KEY!}`,'Content-Type':'application/json',...init.headers},cache:'no-store',signal:AbortSignal.timeout(12000)});
 if(!response.ok)throw new OperationError('Notification storage is unavailable. Verify the reviewed notification schema and server configuration.',503);return response;
}
export async function readNotices():Promise<NoticeState>{
...
export async function publicNotices(){if(mode()!=='demo'&&!configured())return [];return activeNotices((await readNotices()).items);}
export async function saveNotice(method:string,body:Record<string,unknown>){
 ...
 try{const current=await readNotices(),result=mutateNotice(current,method,body);
