...
const ICON = 'https://layeredfx.com/brand/layeredfx_app_icon_email.png';
const SITE = 'https://layeredfx.com';
const greeting = '{{ if .Data.first_name }}Hi {{ .Data.first_name }},{{ else }}Hi there,{{ end }}';
const securityHelp = '<strong style="color:#1f2a22">Didn’t make this change?</strong> Reset your password right away using the button above, then contact us at <a href="mailto:hello@layeredfx.com" style="color:#3f6b1e">hello@layeredfx.com</a>.';
const p = (html, style = '') => `<p style="margin:0 0 16px;font-size:16px;line-height:1.65;color:#4a5446;${style}">${html}</p>`;

function button(label, href = '{{ .ConfirmationURL }}') {
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:10px 0 30px"><tr><td style="border-radius:12px;background:#3f6b1e">
          <a href="${href}" style="display:inline-block;padding:15px 30px;border-radius:12px;color:#ffffff;font-size:16px;font-weight:600;text-decoration:none">${label} &rarr;</a>
        </td></tr></table>
        <p style="margin:0 0 6px;font-size:13px;line-height:1.6;color:#7a8374">Button not working? Copy and paste this link into your browser:</p>
        <p style="margin:0 0 30px;font-size:13px;line-height:1.6;word-break:break-all"><a href="${href}" style="color:#3f6b1e">${href}</a></p>`;
}
function code() {
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:10px 0 30px"><tr><td style="padding:18px 28px;border-radius:14px;background:#f4f7ee;border:1px solid #d7dfc9">
          <span style="font-family:'SFMono-Regular',Consolas,'Liberation Mono',Menlo,monospace;font-size:32px;font-weight:700;letter-spacing:8px;color:#1f2a22">{{ .Token }}</span>
        </td></tr></table>`;
}
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f1f2ec">
<tr><td align="center" style="padding:36px 14px">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px">
  <tr><td align="center" style="padding:0 0 22px">
    <a href="${SITE}" style="text-decoration:none"><img src="${ICON}" width="64" height="64" alt="LayeredFX" style="display:block;border:0;border-radius:16px"></a>
  </td></tr>
  <tr><td style="background:#ffffff;border-radius:20px;overflow:hidden">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="height:6px;line-height:6px;font-size:0;background:#1f2a24">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td width="62%" style="height:6px;background:#1f2a24;font-size:0;line-height:6px">&nbsp;</td><td width="38%" style="height:6px;background:#d6ff41;font-size:0;line-height:6px">&nbsp;</td></tr></table>
      </td></tr>
      <tr><td style="padding:40px 40px 10px">
        <p style="margin:0 0 12px;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#6b7c5c">${kicker}</p>
        <h1 style="margin:0 0 18px;font-size:30px;line-height:1.2;font-weight:500;letter-spacing:-0.5px;color:#1f2a22">${heading}</h1>
        ${body}
      </td></tr>
      <tr><td style="padding:22px 40px 28px;border-top:1px solid #e9ebe3;background:#f8f9f4">
        <p style="margin:0;font-size:14px;line-height:1.6;color:#4a5446">${note}</p>
      </td></tr>
    </table>
  </td></tr>
  <tr><td align="center" style="padding:26px 20px 0;font-size:12px;line-height:1.7;color:#7a8374">
    <strong style="color:#4a5446">LayeredFX</strong> · Beautiful surfaces. Spaces with a little more soul.<br>
    Scottsdale, Arizona · <a href="mailto:hello@layeredfx.com" style="color:#7a8374">hello@layeredfx.com</a> · <a href="${SITE}" style="color:#7a8374">layeredfx.com</a><br>
    ${footerReason}
  </td></tr>
</table>
</td></tr>
...
const templates = [
  ['confirm-signup.html', 'Confirm signup', 'Confirm your LayeredFX account', {
    ...
    note: '<strong style="color:#1f2a22">What you can do next</strong><br>Book a consultation, preview finishes in Wall Studio, and keep every project detail in one place.',
  ...
  ['invite-user.html', 'Invite user', 'You’re invited to LayeredFX', {
    ...
    note: '<strong style="color:#1f2a22">Not expecting this?</strong> You can ignore this email. No account is created unless you accept the invitation.',
  ...
  ['magic-link.html', 'Magic link', 'Your LayeredFX sign-in link', {
    ...
    note: '<strong style="color:#1f2a22">Didn’t ask for this?</strong> You can ignore this email. Nobody can sign in without this link.',
  ...
  ['change-email.html', 'Change email address', 'Confirm your new LayeredFX email address', {
    ...
    body: p(greeting, 'margin-bottom:14px') + p('Confirm <strong style="color:#1f2a22">{{ .NewEmail }}</strong> as the new email address for your LayeredFX account. Your current address, {{ .Email }}, stays active until you confirm.') + button('Confirm new email address'),
    note: '<strong style="color:#1f2a22">Didn’t request this change?</strong> You can ignore this email and your email address will stay the same. If you’re concerned, reply to this email.',
  ...
  ['reset-password.html', 'Reset password', 'Reset your LayeredFX password', {
    ...
    note: '<strong style="color:#1f2a22">Didn’t ask for this?</strong> You can ignore this email. Your password will not change unless you use the link above.',
    footerReason: 'This email was sent to {{ .Email }}.',
  }],
  ['reauthentication.html', 'Reauthentication', '{{ .Token }} is your LayeredFX verification code', {
    ...
    note: '<strong style="color:#1f2a22">Keep this code private.</strong> LayeredFX will never ask you for it by phone or text. If you didn’t request it, you can ignore this email.',
    footerReason: 'This email was sent to {{ .Email }}.',
  }],
  ['password-changed.html', 'Password changed (security notification)', 'Your LayeredFX password was changed', {
    ...
    note: '<strong style="color:#1f2a22">Didn’t make this change?</strong> Reset your password right away using the button above, then contact us at <a href="mailto:hello@layeredfx.com" style="color:#3f6b1e">hello@layeredfx.com</a>.',
  ...
  ['email-changed.html', 'Email address changed (security notification)', 'Your LayeredFX email address was changed', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('The email address for your LayeredFX account was changed from <strong style="color:#1f2a22">{{ .OldEmail }}</strong> to <strong style="color:#1f2a22">{{ .Email }}</strong>. If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
  ['phone-changed.html', 'Phone number changed (security notification)', 'Your LayeredFX phone number was changed', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('The phone number for your LayeredFX account ({{ .Email }}) was changed from <strong style="color:#1f2a22">{{ .OldPhone }}</strong> to <strong style="color:#1f2a22">{{ .Phone }}</strong>. If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
  ['identity-linked.html', 'Sign-in method linked (security notification)', 'A sign-in method was linked to your LayeredFX account', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('Your <strong style="color:#1f2a22">{{ .Provider }}</strong> account was linked as a sign-in method for your LayeredFX account ({{ .Email }}). If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
  ['identity-unlinked.html', 'Sign-in method removed (security notification)', 'A sign-in method was removed from your LayeredFX account', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('Your <strong style="color:#1f2a22">{{ .Provider }}</strong> account was removed as a sign-in method for your LayeredFX account ({{ .Email }}). If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
  ['mfa-added.html', 'MFA method added (security notification)', 'A verification method was added to your LayeredFX account', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('A new sign-in verification method (<strong style="color:#1f2a22">{{ .FactorType }}</strong>) was added to your LayeredFX account. If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
  ['mfa-removed.html', 'MFA method removed (security notification)', 'A verification method was removed from your LayeredFX account', {
    ...
    body: p('Hi there,', 'margin-bottom:14px') + p('The sign-in verification method <strong style="color:#1f2a22">{{ .FactorType }}</strong> was removed from your LayeredFX account. Your account is less protected without it. If you made this change, no action is needed.') + button('Reset your password', `${SITE}/forgot-password`),
    note: securityHelp,
    footerReason: 'Security notices are sent for important changes to your account.',
  }],
];
...
1. Deploy the site first so \`${ICON}\` is reachable (the logo in every email).
