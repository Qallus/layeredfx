...
try{
 ...
 await page.setViewportSize({width:390,height:844});await page.goto(base);await page.getByRole('button',{name:'Open navigation'}).click();await page.getByRole('button',{name:'Services',exact:true}).click();await page.locator('#lfx-menu-services').getByRole('link',{name:'Roman clay',exact:true}).click();await page.getByRole('heading',{name:'Roman clay',exact:true}).waitFor();checks.push('Mobile services menu');
