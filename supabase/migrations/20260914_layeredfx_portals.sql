-- Applied to LayeredFX Supabase (tdywcdgbavfcsywejoka) as layeredfx_portals. Source: docs/migration/sql/portals-review-draft.sql,
-- plus a covering index for the auth.users foreign key.
...
revoke all on public.lfx_portal_accounts from public, anon, authenticated;
...
create index if not exists lfx_portal_accounts_user_id_idx on public.lfx_portal_accounts(user_id);
