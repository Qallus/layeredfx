...
export const fixture={signedIn:true,role:'staff',sent:[],listed:[],emails:[],email:true};
...
export function emailConfigured(){return fixture.email;}
export function staffEmailInput(body){if(typeof body.to!=='string'||!body.to.includes('@'))throw new OperationError('Enter a valid recipient email address.');return {to:body.to,subject:body.subject,text:body.body,requestId:body.requestId};}
export async function sendStaffEmail(email,actor){fixture.emails.push({email,actor});return {id:'em_test'};}
`);
...
code=code.replaceAll('@/lib/operations/server',boundary).replaceAll('@/lib/operations/engine.mjs',engine).replaceAll('@/lib/operations/security.mjs',new URL('../lib/operations/security.mjs',import.meta.url).href).replaceAll('@/lib/communications/twilio',boundary).replaceAll('@/lib/communications/email',boundary);const api=await import(moduleUrl(code));
...
test.beforeEach(()=>Object.assign(fixture,{signedIn:true,role:'staff',sent:[],listed:[],emails:[],email:true}));
...
test('staff email requires sign-in, a staff role and the LayeredFX origin',async()=>{fixture.signedIn=false;assert.equal((await send('POST','email',{to:'client@example.com'})).status,401);fixture.signedIn=true;fixture.role='viewer';assert.equal((await send('POST','email',{to:'client@example.com'})).status,403);fixture.role='staff';assert.equal((await send('POST','email',{to:'client@example.com'},'https://evil.example')).status,403);assert.equal(fixture.emails.length,0);});
test('capabilities report email availability and a valid email is handed to the provider helper',async()=>{fixture.email=false;assert.equal((await (await send('GET','capabilities')).json()).email,false);assert.equal((await send('POST','email',{to:'nope'})).status,400);const response=await send('POST','email',{to:'client@example.com',subject:'Quote',body:'Hello',requestId:'req-12345678'});assert.equal(response.status,201);assert.match((await response.json()).message,/not yet confirmed/);assert.equal(fixture.emails.length,1);assert.equal(fixture.emails[0].actor.id,'staff');});
